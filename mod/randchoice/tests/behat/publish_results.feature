@mod @mod_randchoice
Feature: A teacher can choose one of 4 options for publishing randchoice results
  In order to display randchoice activities outcomes
  As a teacher
  I need to publish the randchoice activity results in different ways
  ...
  Scenario: Do not publish results to students
    ...
    And I follow "Course 1"
    When I choose "Option 1" from "Choice 1" randchoice activity
  ...
  Scenario: Show results to students after they answer
    ...
    And I should not see "Graph display"
    And I follow "Course 1"
    And I choose "Option 1" from "Choice 1" randchoice activity
    And I should see "Your selection: Option 1"
    And I should see "Responses"
    And I should see "Graph display"

  @javascript
  Scenario: Show results to students only after the randchoice is closed
    Given I add a "Choice" to section "1" and I fill the form with:
      | Choice name | Choice 1 |
      | Description | Choice Description |
      | Publish results | Show results to students only after the randchoice is closed |
      | option[0] | Option 1 |
      | option[1] | Option 2 |
    And I log out
    And I log in as "student1"
