<?php
    ...
    $url = new moodle_url('/mod/randchoice/report.php', array('id'=>$id));
    ...
    require_capability('mod/randchoice:readresponses', $context);
    ...
    $strresponses = get_string("responses", "randchoice");
    ...
    if (data_submitted() && $action == 'delete' && has_capability('mod/randchoice:deleteresponses',$context) && confirm_sesskey()) {
    ...
    if (!$download) {
        ...
        $PAGE->set_title(format_string($randchoice->name).": $strresponses");
        $PAGE->set_heading($course->fullname);
        echo $OUTPUT->header();
        echo $OUTPUT->heading($randchoice->name, 2, null);
        /// Check to see if groups are being used in this randchoice
        $groupmode = groups_get_activity_groupmode($cm);
        if ($groupmode) {
            groups_get_activity_group($cm, true);
            groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/randchoice/report.php?id='.$id);
        }
    } else {
        $groupmode = groups_get_activity_groupmode($cm);
    }
    ...
    $users = randchoice_get_response_data($randchoice, $cm, $groupmode, $onlyactive);

    if ($download == "ods" && has_capability('mod/randchoice:downloadresponses', $context)) {
        require_once("$CFG->libdir/odslib.class.php");

    /// Calculate file name
        $filename = clean_filename("$course->shortname ".strip_tags(format_string($randchoice->name,true))).'.ods';
    /// Creating a workbook
    ...
    /// Print names of all the fields
        ...
        $myxls->write_string(0,4,get_string("randchoice","randchoice"));
    ...
    if ($download == "xls" && has_capability('mod/randchoice:downloadresponses', $context)) {
        require_once("$CFG->libdir/excellib.class.php");

    /// Calculate file name
        $filename = clean_filename("$course->shortname ".strip_tags(format_string($randchoice->name,true))).'.xls';
    /// Creating a workbook
