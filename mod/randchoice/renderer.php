...
class mod_randchoice_renderer extends plugin_renderer_base {
            ...
            if (!empty($options['hascapability']) && ($options['hascapability'])) {
                if ($availableoption < 1) {
                    ...
                    $html .= html_writer::empty_tag('input',
                            array('type' => 'submit', 'value' => get_string('savemyrandchoice', 'randchoice'), 'class' => 'button'));
                }

                if (!empty($options['allowupdate']) && ($options['allowupdate'])) {
                    $url = new moodle_url('view.php',
                            array('id' => $coursemoduleid, 'action' => 'delrandchoice', 'sesskey' => sesskey()));
    ...
    /**
     * Returns HTML to display randchoices result
     * @param object $randchoices
     * @param bool $forcepublish
     * @return string
     */
    public function display_result($randchoices, $forcepublish = false) {
        if (empty($forcepublish)) { //allow the publish setting to be overridden
            $forcepublish = $randchoices->publish;
        }

        $displaylayout = $randchoices->display;

        if ($forcepublish) {  //CHOICE_PUBLISH_NAMES
            return $this->display_publish_name_vertical($randchoices);
        } else { //CHOICE_PUBLISH_ANONYMOUS';
            if ($displaylayout == DISPLAY_HORIZONTAL_LAYOUT) {
                return $this->display_publish_anonymous_horizontal($randchoices);
            }
            return $this->display_publish_anonymous_vertical($randchoices);
        }
    }
     ...
     * @return string
     */
    public function display_publish_name_vertical($randchoices) {
        global $PAGE;
        $html ='';
        $html .= html_writer::tag('h3',format_string(get_string("responses", "randchoice")));

        $attributes = array('method'=>'POST');
        $attributes['action'] = new moodle_url($PAGE->url);
        $attributes['id'] = 'attemptsform';

        if ($randchoices->viewresponsecapability) {
            $html .= html_writer::start_tag('form', $attributes);
            $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'id', 'value'=> $randchoices->coursemoduleid));
            $html .= html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'sesskey', 'value'=> sesskey()));
        ...
        $table->summary = get_string('responsesto', 'randchoice', format_string($randchoices->name));
        $table->data = array();

        $count = 0;
        ksort($randchoices->options);
        ...
        $accessiblecell->text = get_string('randchoiceoptions', 'randchoice');
        ...
        $usernumberheader->text = get_string('numberofuser', 'randchoice');
        $columns['usernumber'][] = $usernumberheader;


        foreach ($randchoices->options as $optionid => $options) {
            $celloption = clone($celldefault);
            $cellusernumber = clone($celldefault);
            $cellusernumber->style = 'text-align: center;';

            $celltext = '';
            if ($randchoices->showunanswered && $optionid == 0) {
                $celltext = format_string(get_string('notanswered', 'randchoice'));
            } else if ($optionid > 0) {
                $celltext = format_string($randchoices->options[$optionid]->text);
            }
            $numberofuser = 0;
        ...
        // This extra cell is needed in order to support accessibility for screenreader. MDL-30816
        $accessiblecell = new html_table_cell();
        $accessiblecell->text = get_string('userchoosethisoption', 'randchoice');
            ...
            if ($randchoices->showunanswered || $optionid > 0) {
                if (!empty($options->user)) {
                    ...
                    foreach ($options->user as $user) {
                        ...
                        $userfullname = fullname($user, $randchoices->fullnamecapability);
                        if ($randchoices->viewresponsecapability && $randchoices->deleterepsonsecapability  && $optionid > 0) {
                        ...
                        $userimage = $this->output->user_picture($user, array('courseid'=>$randchoices->courseid));
                        $data .= html_writer::tag('div', $userimage, array('class'=>'image'));

                        $userlink = new moodle_url('/user/view.php', array('id'=>$user->id,'course'=>$randchoices->courseid));
        ...
        $actiondata = '';
        if ($randchoices->viewresponsecapability && $randchoices->deleterepsonsecapability) {
            ...
            $actiondata .= html_writer::tag('label', ' ' . get_string('withselected', 'randchoice') . ' ', array('for'=>'menuaction'));

            $actionurl = new moodle_url($PAGE->url, array('sesskey'=>sesskey(), 'action'=>'delete_confirmation()'));
            $select = new single_select($actionurl, 'action', array('delete'=>get_string('delete')), null, array(''=>get_string('chooseaction', 'randchoice')), 'attemptsform');
     ...
     * @return string
     */
    public function display_publish_anonymous_vertical($randchoices) {
        ...
        $tableheader->text = html_writer::tag('div', get_string('randchoiceoptions', 'randchoice'), array('class' => 'accesshide'));
        $rows['header'][] = $tableheader;

        // graph row header
        $graphheader = clone($headercelldefault);
        $graphheader->text = html_writer::tag('div', get_string('responsesresultgraphheader', 'randchoice'), array('class' => 'accesshide'));
        ...
        $userpercentageheader->text = get_string('numberofuserinpercentage', 'randchoice');
        ...
        foreach ($randchoices->options as $optionid => $option) {
            ...
            if($randchoices->numberofuser > 0) {
               $height = ($CHOICE_COLUMN_HEIGHT * ((float)$numberofuser / (float)$randchoices->numberofuser));
               $percentageamount = ((float)$numberofuser/(float)$randchoices->numberofuser)*100.0;
            }

            $displaygraph = html_writer::tag('img','', array('style'=>'height:'.$height.'px;width:49px;', 'alt'=>'', 'src'=>$this->output->pix_url('column', 'randchoice')));
            ...
            $usernumber .= html_writer::tag('div', ' '.$numberofuser.'', array('class'=>'numberofuser', 'title'=> get_string('numberofuser', 'randchoice')));
        ...
        $header = html_writer::tag('h3',format_string(get_string("responses", "randchoice")));
        $html .= html_writer::tag('div', $header, array('class'=>'responseheader'));
        $html .= html_writer::tag('a', get_string('skipresultgraph', 'randchoice'), array('href'=>'#skipresultgraph', 'class'=>'skip-block'));
        $html .= html_writer::tag('div', html_writer::table($table), array('class'=>'response'));
     ...
     * @return string
     */
    public function display_publish_anonymous_horizontal($randchoices) {
        ...
        $tableheadertext->text = get_string('randchoiceoptions', 'randchoice');

        $tableheadernumber = clone($columnheaderdefault);
        $tableheadernumber->text = get_string('numberofuser', 'randchoice');

        $tableheaderpercentage = clone($columnheaderdefault);
        $tableheaderpercentage->text = get_string('numberofuserinpercentage', 'randchoice');

        $tableheadergraph = clone($columnheaderdefault);
        $tableheadergraph->text = get_string('responsesresultgraphheader', 'randchoice');
        ...
        foreach ($randchoices->options as $optionid => $options) {
            ...
            if($randchoices->numberofuser > 0) {
               $width = ($CHOICE_COLUMN_WIDTH * ((float)$numberofuser / (float)$randchoices->numberofuser));
               $percentageamount = ((float)$numberofuser/(float)$randchoices->numberofuser)*100.0;
            }
            ...
            $attributes['alt'] = '';
            $attributes['src'] = $this->output->pix_url('row', 'randchoice');
