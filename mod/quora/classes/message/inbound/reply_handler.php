...
/**
 * A Handler to process replies to quora posts.
 *
 * @package    mod_quora
...
namespace mod_quora\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'mod_quora');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'mod_quora');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (empty($quora->maxattachments) || $quora->maxbytes == 1 ||
                    !has_capability('mod/quora:createattachment', $modcontext)) {
            ...
            if ($quora->maxattachments < $attachmentcount) {
                ...
                mtrace("--> User attached {$attachmentcount} files when only {$quora->maxattachments} where allowed. "
                     . " Rejecting e-mail.");
            ...
            if ($quora->maxbytes < $filesize) {
                // Too many attachments.
                mtrace("--> User attached {$filesize} bytes of files when only {$quora->maxbytes} where allowed. "
                     . "Rejecting e-mail.");
                $data = new \stdClass();
                $data->quora = $quora;
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/mod/quora/discuss.php', array('d' => $handlerresult->discussion));
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'mod_quora', $a);
        $message->html = get_string('postbymailsuccess_html', 'mod_quora', $a);
        return $message;
    }
}
