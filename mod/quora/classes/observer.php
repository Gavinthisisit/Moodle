...
/**
 * Event observer for mod_quora.
 */
class mod_quora_observer {
    ...
    public static function user_enrolment_deleted(\core\event\user_enrolment_deleted $event) {
        ...
        if ($cp->lastenrol) {
            ...
            $quoraselect = "IN (SELECT f.id FROM {quora} f WHERE f.course = :courseid)";

            $DB->delete_records_select('quora_digests', 'userid = :userid AND quora '.$quoraselect, $params);
            $DB->delete_records_select('quora_subscriptions', 'userid = :userid AND quora '.$quoraselect, $params);
            $DB->delete_records_select('quora_track_prefs', 'userid = :userid AND quoraid '.$quoraselect, $params);
            $DB->delete_records_select('quora_read', 'userid = :userid AND quoraid '.$quoraselect, $params);
        }
    }
    ...
    public static function role_assigned(\core\event\role_assigned $event) {
        ...
        // at course level means user is enroled in course and can subscribe to quora.
        ...
        foreach ($quoras as $quora) {
            ...
            $modcontext = context_module::instance($quora->cmid);
            if (has_capability('mod/quora:allowforcesubscribe', $modcontext, $userid)) {
                \mod_quora\subscriptions::subscribe_user($userid, $quora, $modcontext);
            }
        }
    }
