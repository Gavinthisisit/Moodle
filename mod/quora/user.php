...
$url = new moodle_url('/mod/quora/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
if ($isspecificcourse) {
    ...
    $courses = quora_get_courses_user_posted_in($user, $discussionsonly);
}
...
$result = quora_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_quora');

    // Get the page heading
    if ($isspecificcourse) {
        $pageheading = format_string($course->fullname, true, array('context' => $coursecontext));
    } else {
        $pageheading = get_string('pluginname', 'mod_quora');
    }
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'quora');
        } else {
            $notification = get_string('nopostsmadebyyou', 'quora');
        }
        // These are the user's quora interactions.
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'quora'), new moodle_url('/mod/quora/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'quora'), new moodle_url('/mod/quora/user.php',
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'quora', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'quora', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'quora');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('quora_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular quora,
...
foreach ($result->posts as $post) {
    if (!isset($result->quoras[$post->quora]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $quora = $result->quoras[$post->quora];
    $cm = $quora->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $quoraurl = new moodle_url('/mod/quora/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/quora/discuss.php', array('d' => $post->discussion));
    ...
    if ($quora->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($quora->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $quoraurl;
        } else {
    ...
    $quoraname = format_string($quora->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($quoraurl, $quoraname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $quoraname);
    }
...
if ($isspecificcourse) {
    ...
    if ($discussionsonly) {
        ...
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_quora', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'quora', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'quora'));
}
