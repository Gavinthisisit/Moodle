...
/**
 * The module quoras external functions unit tests
 *
 * @package    mod_quora
 * @category   external
...
class mod_quora_external_testcase extends externallib_advanced_testcase {
    ...
    protected function setUp() {
        ...
        require_once($CFG->dirroot . '/mod/quora/externallib.php');
    }
     ...
     * Test get quoras
     */
    public function test_mod_quora_get_quoras_by_courses() {
        global $USER, $CFG, $DB;
        ...
        $quora1 = self::getDataGenerator()->create_module('quora', $record);

        // Second quora.
        $record = new stdClass();
        $record->introformat = FORMAT_HTML;
        $record->course = $course2->id;
        $quora2 = self::getDataGenerator()->create_module('quora', $record);

        // Add discussions to the quoras.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->quora = $quora1->id;
        $discussion1 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_discussion($record);
        // Expect one discussion.
        $quora1->numdiscussions = 1;

        $record = new stdClass();
        $record->course = $course2->id;
        $record->userid = $user->id;
        $record->quora = $quora2->id;
        $discussion2 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_discussion($record);
        $discussion3 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_discussion($record);
        // Expect two discussions.
        $quora2->numdiscussions = 2;

        // Check the quora was correctly created.
        $this->assertEquals(2, $DB->count_records_select('quora', 'id = :quora1 OR id = :quora2',
                array('quora1' => $quora1->id, 'quora2' => $quora2->id)));
        ...
        // Assign capabilities to view quoras for quora 2.
        $cm2 = get_coursemodule_from_id('quora', $quora2->cmid, 0, false, MUST_EXIST);
        $context2 = context_module::instance($cm2->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $roleid2 = $this->assignUserCapability('mod/quora:viewdiscussion', $context2->id, $newrole);

        // Create what we expect to be returned when querying the two courses.
        unset($quora1->displaywordcount);
        unset($quora2->displaywordcount);

        $expectedquoras = array();
        $expectedquoras[$quora1->id] = (array) $quora1;
        $expectedquoras[$quora2->id] = (array) $quora2;

        // Call the external function passing course ids.
        $quoras = mod_quora_external::get_quoras_by_courses(array($course1->id, $course2->id));
        $quoras = external_api::clean_returnvalue(mod_quora_external::get_quoras_by_courses_returns(), $quoras);
        $this->assertCount(2, $quoras);
        foreach ($quoras as $quora) {
            $this->assertEquals($expectedquoras[$quora['id']], $quora);
        }

        // Call the external function without passing course id.
        $quoras = mod_quora_external::get_quoras_by_courses();
        $quoras = external_api::clean_returnvalue(mod_quora_external::get_quoras_by_courses_returns(), $quoras);
        $this->assertCount(2, $quoras);
        foreach ($quoras as $quora) {
            $this->assertEquals($expectedquoras[$quora['id']], $quora);
        ...
        // Unenrol user from second course and alter expected quoras.
        $enrol->unenrol_user($instance2, $user->id);
        unset($expectedquoras[$quora2->id]);

        // Call the external function without passing course id.
        $quoras = mod_quora_external::get_quoras_by_courses();
        $quoras = external_api::clean_returnvalue(mod_quora_external::get_quoras_by_courses_returns(), $quoras);
        $this->assertCount(1, $quoras);
        $this->assertEquals($expectedquoras[$quora1->id], $quoras[0]);

        // Call for the second course we unenrolled the user from.
        $quoras = mod_quora_external::get_quoras_by_courses(array($course2->id));
        $quoras = external_api::clean_returnvalue(mod_quora_external::get_quoras_by_courses_returns(), $quoras);
        $this->assertCount(0, $quoras);
    }

    /**
     * Test get quora discussions
     */
    public function test_mod_quora_get_quora_discussions() {
        global $USER, $CFG, $DB;

        $this->resetAfterTest(true);

        // Set the CFG variable to allow track quoras.
        $CFG->quora_trackreadposts = true;

        // Create a user who can track quoras.
        $record = new stdClass();
        $record->trackquoras = true;
        ...
        // First quora with tracking off.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->trackingtype = FORUM_TRACKING_OFF;
        $quora1 = self::getDataGenerator()->create_module('quora', $record);

        // Second quora of type 'qanda' with tracking enabled.
        $record = new stdClass();
        $record->course = $course2->id;
        $record->type = 'qanda';
        ...
        $record->userid = $user2->id;
        $discussion1reply1 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_post($record);

        $record->parent = $discussion1reply1->id;
        $record->userid = $user3->id;
        $discussion1reply2 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_post($record);

        $record->userid = $user4->id;
        $discussion1reply3 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_post($record);
        ...
        $record->userid = $user1->id;
        $discussion2reply1 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_post($record);

        $record->parent = $discussion2reply1->id;
        $record->userid = $user3->id;
        $discussion2reply2 = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_post($record);

        // Check the quoras were correctly created.
        $this->assertEquals(2, $DB->count_records_select('quora', 'id = :quora1 OR id = :quora2',
                array('quora1' => $quora1->id, 'quora2' => $quora2->id)));

        // Check the discussions were correctly created.
        $this->assertEquals(2, $DB->count_records_select('quora_discussions', 'quora = :quora1 OR quora = :quora2',
                                                            array('quora1' => $quora1->id, 'quora2' => $quora2->id)));

        // Check the posts were correctly created, don't forget each discussion created also creates a post.
        $this->assertEquals(7, $DB->count_records_select('quora_posts', 'discussion = :discussion1 OR discussion = :discussion2',
        ...
        // So the user automatically gets mod/quora:viewdiscussion on all quoras of the course.
        ...
        // Assign capabilities to view discussions for quora 2.
        $cm = get_coursemodule_from_id('quora', $quora2->cmid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $this->assignUserCapability('mod/quora:viewdiscussion', $context->id, $newrole);

        // Create what we expect to be returned when querying the quoras.
        $expecteddiscussions = array();
        $expecteddiscussions[] = array(
                'id' => $discussion1->id,
                'course' => $discussion1->course,
                'quora' => $discussion1->quora,
        ...
        // Call the external function passing quora ids.
        $discussions = mod_quora_external::get_quora_discussions(array($quora1->id, $quora2->id));
        $discussions = external_api::clean_returnvalue(mod_quora_external::get_quora_discussions_returns(), $discussions);
        $this->assertEquals($expecteddiscussions, $discussions);
        // Some debugging is going to be produced, this is because we switch PAGE contexts in the get_quora_discussions function,
        ...
        // Remove the users post from the qanda quora and ensure they can still see the discussion.
        $DB->delete_records('quora_posts', array('id' => $discussion2reply1->id));
        $discussions = mod_quora_external::get_quora_discussions(array($quora2->id));
        $discussions = external_api::clean_returnvalue(mod_quora_external::get_quora_discussions_returns(), $discussions);
        $this->assertEquals(1, count($discussions));

        // Call without required view discussion capability.
        $this->unassignUserCapability('mod/quora:viewdiscussion', null, null, $course1->id);
        try {
            mod_quora_external::get_quora_discussions(array($quora1->id));
        ...
        try {
            mod_quora_external::get_quora_discussions(array($quora2->id));
    ...
    /**
     * Test get quora posts
     */
    public function test_mod_quora_get_quora_discussion_posts() {
        global $CFG;
        ...
        $record->trackingtype = FORUM_TRACKING_OFF;
        $quora1 = self::getDataGenerator()->create_module('quora', $record);
        $quora1context = context_module::instance($quora1->cmid);
        ...
        $expectedposts['posts'][] = array(
            ...
            'message' => file_rewrite_pluginfile_urls($discussion1reply2->message, 'pluginfile.php',
                    $quora1context->id, 'mod_quora', 'post', $discussion1reply2->id),
        ...
        $expectedposts['posts'][] = array(
            ...
            'message' => file_rewrite_pluginfile_urls($discussion1reply1->message, 'pluginfile.php',
                    $quora1context->id, 'mod_quora', 'post', $discussion1reply1->id),
            'messageformat' => 1,   // This value is usually changed by external_format_text() function.
        ...
        $posts = mod_quora_external::get_quora_discussion_posts($discussion1->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_quora_external::get_quora_discussion_posts_returns(), $posts);
        ...
        $posts = mod_quora_external::get_quora_discussion_posts($discussion2->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_quora_external::get_quora_discussion_posts_returns(), $posts);
    ...
    /**
     * Test get quora posts (qanda quora)
     */
    public function test_mod_quora_get_quora_discussion_posts_qanda() {
        global $CFG, $DB;
        ...
        $DB->update_record('quora_posts', $discussion1reply2);
    ...
    /**
     * Test get quora discussions paginated
     */
    public function test_mod_quora_get_quora_discussions_paginated() {
        global $USER, $CFG, $DB;
        ...
        // Assign capabilities to view discussions for quora 1.
        $cm = get_coursemodule_from_id('quora', $quora1->cmid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $this->assignUserCapability('mod/quora:viewdiscussion', $context->id, $newrole);
        ...
        $post1 = $DB->get_record('quora_posts', array('id' => $discussion1->firstpost), '*', MUST_EXIST);
        $userpictureurl = moodle_url::make_webservice_pluginfile_url(
        ...
        // Call the external function passing quora id.
        $discussions = mod_quora_external::get_quora_discussions_paginated($quora1->id);
        $discussions = external_api::clean_returnvalue(mod_quora_external::get_quora_discussions_paginated_returns(), $discussions);
        ...
        // Call without required view discussion capability.
        $this->unassignUserCapability('mod/quora:viewdiscussion', $context->id, $newrole);
        try {
            mod_quora_external::get_quora_discussions_paginated($quora1->id);
            $this->fail('Exception expected due to missing capability.');
        } catch (moodle_exception $e) {
    ...
    /**
     * Test get quora discussions paginated (qanda quoras)
     */
    public function test_mod_quora_get_quora_discussions_paginated_qanda() {
        ...
        $discussionrecord->quora = $quora->id;
        $discussion = self::getDataGenerator()->get_plugin_generator('mod_quora')->create_discussion($discussionrecord);

        self::setAdminUser();
        $discussions = mod_quora_external::get_quora_discussions_paginated($quora->id);
        $discussions = external_api::clean_returnvalue(mod_quora_external::get_quora_discussions_paginated_returns(), $discussions);
