...
class mod_quora_maildigest_testcase extends advanced_testcase {
    ...
    /**
     * Setup a user, course, and quoras.
     *
     * @return stdClass containing the list of quoras, courses, quoraids,
    ...
    protected function helper_setup_user_in_course() {
        ...
        $return->quoras = new stdClass();
        $return->quoraids = array();
        ...
        $return->quoras->quora1 = $this->getDataGenerator()->create_module('quora', $record);
        $return->quorasids[] = $return->quoras->quora1->id;

        $return->quoras->quora2 = $this->getDataGenerator()->create_module('quora', $record);
        $return->quorasids[] = $return->quoras->quora2->id;

        // Check the quora was correctly created.
        list ($test, $params) = $DB->get_in_or_equal($return->quorasids);
    ...
    /**
     * Helper to falsify all quora post records for a digest run.
     */
    protected function helper_force_digest_mail_times() {
        ...
        $DB->set_field('quora_posts', 'modified', $digesttime, array('mailed' => 0));
        $DB->set_field('quora_posts', 'created', $digesttime, array('mailed' => 0));
    }
    ...
    public function test_set_maildigest() {
        ...
        $currentsetting = $DB->get_record('quora_digests', array(
            'quora' => $quora1->id,
            'userid' => $user->id,
        ));
        ...
        quora_set_user_maildigest($quora1, 0, $user);
        $currentsetting = $DB->get_record('quora_digests', array(
            'quora' => $quora1->id,
            'userid' => $user->id,
        ));
        ...
        quora_set_user_maildigest($quora1, 1, $user);
        $currentsetting = $DB->get_record('quora_digests', array(
            'quora' => $quora1->id,
            'userid' => $user->id,
        ));
        ...
        quora_set_user_maildigest($quora1, 2, $user);
        $currentsetting = $DB->get_record('quora_digests', array(
            'quora' => $quora1->id,
            'userid' => $user->id,
        ));
        ...
        quora_set_user_maildigest($quora1, 42, $user);
    }
    ...
    public function test_get_user_digest_options_default() {
        ...
        $digestoptions = array(
            '0' => get_string('emaildigestoffshort', 'mod_quora'),
            '1' => get_string('emaildigestcompleteshort', 'mod_quora'),
            '2' => get_string('emaildigestsubjectsshort', 'mod_quora'),
        );

        // The default settings is 0.
        $this->assertEquals(0, $user->maildigest);
        $options = quora_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_quora', $digestoptions[0]));

        // Update the setting to 1.
        $USER->maildigest = 1;
        $this->assertEquals(1, $USER->maildigest);
        $options = quora_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_quora', $digestoptions[1]));

        // Update the setting to 2.
        $USER->maildigest = 2;
        $this->assertEquals(2, $USER->maildigest);
        $options = quora_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_quora', $digestoptions[2]));
    }
    ...
    public function test_cron_no_posts() {
        ...
        // Initially the quora cron should generate no messages as we've made no posts.
    ...
    public function test_cron_profile_single_mails() {
        ...
        $quora1 = $userhelper->quoras->quora1;
        $quora2 = $userhelper->quoras->quora2;

        // Add some discussions to the quoras.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->mailnow = 1;

        // Add 5 discussions to quora 1.
        $record->quora = $quora1->id;
        for ($i = 0; $i < 5; $i++) {
            $this->getDataGenerator()->get_plugin_generator('mod_quora')->create_discussion($record);
        }

        // Add 5 discussions to quora 2.
        ...
        // Set the maildigest preference for quora1 to default.
        quora_set_user_maildigest($quora1, -1, $user);

        // Set the maildigest preference for quora2 to default.
        quora_set_user_maildigest($quora2, -1, $user);

        // No digests mails should be sent, but 10 quora mails will be sent.
    ...
    public function test_cron_profile_digest_email() {
        ...
        // Add a discussion to the quoras.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->mailnow = 1;
    ...
    /**
     * Sends several notifications to one user as:
     * * daily digests coming from the per-quora setting; and
    ...
    public function test_cron_mixed_email_1() {
        ...
        // Set the maildigest preference for quora1 to digest.
        quora_set_user_maildigest($quora1, 1, $user);

        // Set the maildigest preference for quora2 to default (single).
        quora_set_user_maildigest($quora2, -1, $user);
    ...
    /**
     * Sends several notifications to one user as:
     * * single e-mails from the per-quora setting; and
    ...
    public function test_cron_mixed_email_2() {
        ...
        // Set the maildigest preference for quora2 to single.
        quora_set_user_maildigest($quora2, 0, $user);
    ...
    /**
     * Sends several notifications to one user as:
     * * daily digests coming from the per-quora setting.
     */
    public function test_cron_quora_digest_email() {
        global $DB, $CFG;
        ...
        // Set the maildigest preference for quora1 to digest (complete).
        quora_set_user_maildigest($quora1, 1, $user);

        // Set the maildigest preference for quora2 to digest (short).
        quora_set_user_maildigest($quora2, 2, $user);
