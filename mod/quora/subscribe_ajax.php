...
/**
 * Subscribe to or unsubscribe from a quora discussion.
 *
 * @package    mod_quora
...
$quoraid        = required_param('quoraid', PARAM_INT);             // The quora to subscribe or unsubscribe.
...
require_capability('mod/quora:viewdiscussion', $context);
...
if (is_guest($context, $USER)) {
    ...
    throw new moodle_exception('noguestsubscribe', 'mod_quora');
}
...
if (\mod_quora\subscriptions::is_subscribed($USER->id, $quora, $discussion->id, $cm)) {
    // The user is subscribed, unsubscribe them.
    \mod_quora\subscriptions::unsubscribe_user_from_discussion($USER->id, $discussion, $context);
} else {
...
$return->icon = quora_get_discussion_subscription_icon($quora, $discussion->id, null, $includetext);
echo json_encode($return);
die;
