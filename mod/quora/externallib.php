...
class mod_quora_external extends external_api {

    /**
     * Describes the parameters for get_quora.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.5
     */
    public static function get_quoras_by_courses_parameters() {
        ...
        if (!empty($courseids)) {
            ...
            if ($quoras = get_all_instances_in_courses("quora", $dbcourses)) {
                foreach ($quoras as $quora) {
                    ...
                    list($quora->intro, $quora->introformat) = external_format_text($quora->intro, $quora->introformat,
                                                                                    $context->id, 'mod_quora', 'intro', 0);
    ...
    /**
     * Describes the get_quora return value.
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
     public static function get_quoras_by_courses_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'Forum id'),
                    'course' => new external_value(PARAM_TEXT, 'Course id'),
                    'type' => new external_value(PARAM_TEXT, 'The quora type'),
                    'name' => new external_value(PARAM_TEXT, 'Forum name'),
                    'intro' => new external_value(PARAM_RAW, 'The quora intro'),
                    ...
                    'numdiscussions' => new external_value(PARAM_INT, 'Number of discussions in the quora', VALUE_OPTIONAL)
                ), 'quora'
            )
        );
    }
    ...
    /**
     * Describes the parameters for get_quora_discussions.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.5
     * @deprecated Moodle 2.8 MDL-46458 - Please do not call this function any more.
     * @see get_quora_discussions_paginated
     */
    public static function get_quora_discussions_parameters() {
        return new external_function_parameters (
            array(
                'quoraids' => new external_multiple_structure(new external_value(PARAM_INT, 'quora ID',
    ...
    /**
     * Returns a list of quora discussions as well as a summary of the discussion
     * in a provided list of quoras.
     *
     * @param array $quoraids the quora ids
     * @param int $limitfrom limit from SQL data
     * @param int $limitnum limit number SQL data
     *
     * @return array the quora discussion details
     * @since Moodle 2.5
     * @deprecated Moodle 2.8 MDL-46458 - Please do not call this function any more.
     * @see get_quora_discussions_paginated
     */
    public static function get_quora_discussions($quoraids, $limitfrom = 0, $limitnum = 0) {
        global $CFG, $DB, $USER;

        require_once($CFG->dirroot . "/mod/quora/lib.php");

        // Validate the parameter.
        $params = self::validate_parameters(self::get_quora_discussions_parameters(),
            array(
                'quoraids'  => $quoraids,
                'limitfrom' => $limitfrom,
                'limitnum'  => $limitnum,
            ));
        $quoraids  = $params['quoraids'];
        $limitfrom = $params['limitfrom'];
        $limitnum  = $params['limitnum'];

        // Array to store the quora discussions to return.
        $arrdiscussions = array();
        // Keep track of the users we have looked up in the DB.
        $arrusers = array();

        // Loop through them.
        foreach ($quoraids as $id) {
            // Get the quora object.
            $quora = $DB->get_record('quora', array('id' => $id), '*', MUST_EXIST);
            $course = get_course($quora->course);

            $modinfo = get_fast_modinfo($course);
            $quoras  = $modinfo->get_instances_of('quora');
            $cm = $quoras[$quora->id];
            ...
            require_capability('mod/quora:viewdiscussion', $modcontext);

            // Get the discussions for this quora.
            $params = array();
            ...
            $select = "quora = ? $groupselect";

            if ($discussions = $DB->get_records_select('quora_discussions', $select, $params, 'timemodified DESC', '*',
                                                            $limitfrom, $limitnum)) {

                // Check if they can view full names.
                $canviewfullname = has_capability('moodle/site:viewfullnames', $modcontext);
                // Get the unreads array, this takes a quora id and returns data for all discussions.
                $unreads = array();
                if ($cantrack = quora_tp_can_track_quoras($quora)) {
                    if ($quoratracked = quora_tp_is_tracked($quora)) {
                        $unreads = quora_get_discussions_unread($cm);
                    }
                }
                // The quora function returns the replies for all the discussions in a given quora.
                $replies = quora_count_discussion_replies($id);

                foreach ($discussions as $discussion) {
                    // This function checks capabilities, timed discussions, groups and qanda quoras posting.
                    if (!quora_user_can_see_discussion($quora, $discussion, $modcontext)) {
                        continue;
                    }
                    ...
                    // Get the subject.
                    $subject = $DB->get_field('quora_posts', 'subject', array('id' => $discussion->firstpost), MUST_EXIST);
                    // Create object to return.
                    $return = new stdClass();
                    $return->id = (int) $discussion->id;
                    $return->course = $discussion->course;
                    $return->quora = $discussion->quora;
                    ...
                    $lastpost = $DB->get_record('quora_posts', array('id' => $return->lastpost), '*', MUST_EXIST);
    ...
    /**
     * Describes the get_quora_discussions return value.
     *
     * @return external_single_structure
     * @since Moodle 2.5
     * @deprecated Moodle 2.8 MDL-46458 - Please do not call this function any more.
     * @see get_quora_discussions_paginated
     */
     public static function get_quora_discussions_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'Forum id'),
                    'course' => new external_value(PARAM_INT, 'Course id'),
                    'quora' => new external_value(PARAM_INT, 'The quora id'),
                    ...
                    'numunread' => new external_value(PARAM_TEXT, 'The number of unread posts, blank if this value is
                        not available due to quora settings.'),
    ...
    /**
     * Describes the parameters for get_quora_discussion_posts.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.7
     */
    public static function get_quora_discussion_posts_parameters() {
        return new external_function_parameters (
            array(
    ...
    public static function get_quora_discussion_posts($discussionid, $sortby = "created", $sortdirection = "DESC") {
        ...
        // Validate the parameter.
        $params = self::validate_parameters(self::get_quora_discussion_posts_parameters(),
            array(
        ...
        foreach ($allposts as $post) {
            ...
            list($post->message, $post->messageformat) =
                external_format_text($post->message, $post->messageformat, $modcontext->id, 'mod_quora', 'post', $post->id);
                ...
                if ($files = $fs->get_area_files($modcontext->id, 'mod_quora', 'attachment', $post->id, "filename", false)) {
                    foreach ($files as $file) {
                        ...
                        $fileurl = moodle_url::make_webservice_pluginfile_url(
                                        $modcontext->id, 'mod_quora', 'attachment', $post->id, '/', $filename);
    ...
    /**
     * Describes the get_quora_discussion_posts return value.
     *
     * @return external_single_structure
     * @since Moodle 2.7
     */
    public static function get_quora_discussion_posts_returns() {
    ...
    /**
     * Describes the parameters for get_quora_discussions_paginated.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.8
     */
    public static function get_quora_discussions_paginated_parameters() {
        return new external_function_parameters (
            array(
                'quoraid' => new external_value(PARAM_INT, 'quora instance id', VALUE_REQUIRED),
                'sortby' => new external_value(PARAM_ALPHA,
    ...
    /**
     * Returns a list of quora discussions optionally sorted and paginated.
     *
     * @param int $quoraid the quora instance id
     * @param string $sortby sort by this element (id, timemodified, timestart or timeend)
     * @param string $sortdirection sort direction: ASC or DESC
     * @param int $page page number
     * @param int $perpage items per page
     *
     * @return array the quora discussion details including warnings
     * @since Moodle 2.8
     */
    public static function get_quora_discussions_paginated($quoraid, $sortby = 'timemodified', $sortdirection = 'DESC',
                                                    $page = -1, $perpage = 0) {
        global $CFG, $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_quora_discussions_paginated_parameters(),
            array(
                'quoraid' => $quoraid,
                'sortby' => $sortby,
                'sortdirection' => $sortdirection,
        ...
        $alldiscussions = quora_get_discussions($cm, $sort, true, -1, -1, true, $page, $perpage);
        ...
        if ($alldiscussions) {
            ...
            // The quora function returns the replies for all the discussions in a given quora.
            $replies = quora_count_discussion_replies($quoraid, $sort, -1, $page, $perpage);

            foreach ($alldiscussions as $discussion) {

                // This function checks for qanda quoras.
                // Note that the quora_get_discussions returns as id the post id, not the discussion id so we need to do this.
                $discussionrec = clone $discussion;
                $discussionrec->id = $discussion->discussion;
                if (!quora_user_can_see_discussion($quora, $discussionrec, $modcontext)) {
                    $warning = array();
                    // Function quora_get_discussions returns quora_posts ids not quora_discussions ones.
                    $warning['item'] = 'post';
                ...
                list($discussion->message, $discussion->messageformat) =
                    external_format_text($discussion->message, $discussion->messageformat,
                                            $modcontext->id, 'mod_quora', 'post', $discussion->id);
                ...
                if (!empty($discussion->attachment)) {
                    ...
                    $fs = get_file_storage();
                    if ($files = $fs->get_area_files($modcontext->id, 'mod_quora', 'attachment',
                        ...
                        foreach ($files as $file) {
                            ...
                            $discussion->attachments[] = array(
                                ...
                                'fileurl'  => file_encode_url($CFG->wwwroot.'/webservice/pluginfile.php',
                                                '/'.$modcontext->id.'/mod_quora/attachment/'.$discussion->id.'/'.$filename)
                            );
                        }
                    }
                }
    ...
    /**
     * Describes the get_quora_discussions_paginated return value.
     *
     * @return external_single_structure
     * @since Moodle 2.8
     */
    public static function get_quora_discussions_paginated_returns() {
        return new external_single_structure(
            array(
     ...
     * @since Moodle 2.9
     */
    public static function view_quora_parameters() {
        return new external_function_parameters(
            array(
                'quoraid' => new external_value(PARAM_INT, 'quora instance id')
            )
        );
    }

    /**
     * Simulate the quora/view.php web interface page: trigger events, completion, etc...
     *
     * @param int $quoraid the quora instance id
     ...
     */
    public static function view_quora($quoraid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/quora/lib.php");

        $params = self::validate_parameters(self::view_quora_parameters(),
                                            array(
                                                'quoraid' => $quoraid
                                            ));
        $warnings = array();
        ...
        $quora = $DB->get_record('quora', array('id' => $params['quoraid']), 'id', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($quora, 'quora');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        require_capability('mod/quora:viewdiscussion', $context, null, true, 'noviewdiscussionspermission', 'quora');

        // Call the quora/lib API.
     ...
     * @since Moodle 2.9
     */
    public static function view_quora_returns() {
        return new external_single_structure(
            array(
     ...
     * @since Moodle 2.9
     */
    public static function view_quora_discussion_parameters() {
        return new external_function_parameters(
            array(
    ...
    /**
     * Simulate the quora/discuss.php web interface page: trigger events
     *
     ...
     * @throws moodle_exception
     */
    public static function view_quora_discussion($discussionid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/quora/lib.php");

        $params = self::validate_parameters(self::view_quora_discussion_parameters(),
                                            array(
                                                'discussionid' => $discussionid
                                            ));
        $warnings = array();

        $discussion = $DB->get_record('quora_discussions', array('id' => $params['discussionid']), '*', MUST_EXIST);
        $quora = $DB->get_record('quora', array('id' => $discussion->quora), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($quora, 'quora');
     ...
     * @since Moodle 2.9
     */
    public static function view_quora_discussion_returns() {
        return new external_single_structure(
            array(
                'status' => new external_value(PARAM_BOOL, 'status: true if success'),
                'warnings' => new external_warnings()
