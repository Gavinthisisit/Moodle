...
$url = new moodle_url('/mod/twf/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
if ($isspecificcourse) {
    ...
    $courses = twf_get_courses_user_posted_in($user, $discussionsonly);
}
...
$result = twf_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'twf');
        } else {
            $notification = get_string('nopostsmadebyyou', 'twf');
        }
        // These are the user's twf interactions.
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'twf'), new moodle_url('/mod/twf/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'twf'), new moodle_url('/mod/twf/user.php',
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'twf', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'twf', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'twf');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('twf_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular twf,
...
foreach ($result->posts as $post) {
    if (!isset($result->twfs[$post->twf]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $twf = $result->twfs[$post->twf];
    $cm = $twf->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $twfurl = new moodle_url('/mod/twf/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/twf/discuss.php', array('d' => $post->discussion));
    ...
    if ($twf->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($twf->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $twfurl;
        } else {
    ...
    $twfname = format_string($twf->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($twfurl, $twfname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $twfname);
    }
    if ($twf->type != 'single') {
...
if ($isspecificcourse) {
    ...
    if ($discussionsonly) {
        ...
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_twf', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'twf', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'twf'));
}
