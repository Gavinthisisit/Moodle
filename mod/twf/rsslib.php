 ...
 */
function twf_rss_get_feed($context, $args) {
    global $CFG, $DB, $USER;

    $status = true;

    //are RSS feeds enabled?
    if (empty($CFG->twf_enablerssfeeds)) {
        debugging('DISABLED (module configuration)');
        return null;
    }

    $twfid  = clean_param($args[3], PARAM_INT);
    $cm = get_coursemodule_from_instance('twf', $twfid, 0, false, MUST_EXIST);
    $modcontext = context_module::instance($cm->id);

    //context id from db should match the submitted one
    if ($context->id != $modcontext->id || !has_capability('mod/twf:viewdiscussion', $modcontext)) {
        return null;
    }

    $twf = $DB->get_record('twf', array('id' => $twfid), '*', MUST_EXIST);
    if (!rss_enabled_for_mod('twf', $twf)) {
        return null;
    }

    //the sql that will retreive the data for the feed and be hashed to get the cache filename
    list($sql, $params) = twf_rss_get_sql($twf, $cm);

    // Hash the sql to get the cache file name.
    $filename = rss_get_file_name($twf, $sql, $params);
    $cachedfilepath = rss_get_file_full_name('mod_twf', $filename);
    ...
    if (($cachedfilelastmodified == 0) || (($dontrecheckcutoff > $cachedfilelastmodified) &&
        twf_rss_newstuff($twf, $cm, $cachedfilelastmodified))) {
        // Need to regenerate the cached version.
        $result = twf_rss_feed_contents($twf, $sql, $params, $modcontext);
        $status = rss_save_file('mod_twf', $filename, $result);
    }
...
/**
 * If there is new stuff in the twf since $time this returns true
 ...
 */
function twf_rss_newstuff($twf, $cm, $time) {
    global $DB;

    list($sql, $params) = twf_rss_get_sql($twf, $cm, $time);
 ...
 * @param stdClass $cm    Course Module object
 * @param int      $time  check for items since this epoch timestamp
 * @return string the SQL query to be used to get the Discussion/Post details from the twf table of the database
 */
function twf_rss_get_sql($twf, $cm, $time=0) {
    if ($twf->rsstype == 1) { // Discussion RSS
        return twf_rss_feed_discussions_sql($twf, $cm, $time);
    } else { // Post RSS
        return twf_rss_feed_posts_sql($twf, $cm, $time);
    }
}

/**
 * Generates the SQL query used to get the Discussion details from the twf table of the database
 *
 * @param stdClass $twf     the twf object
 * @param stdClass $cm        Course Module object
 * @param int      $newsince  check for items since this epoch timestamp
 * @return string the SQL query to be used to get the Discussion details from the twf table of the database
 */
function twf_rss_feed_discussions_sql($twf, $cm, $newsince=0) {
    global $CFG, $DB, $USER;
    ...
    list($groupselect, $groupparams) = twf_rss_get_group_sql($cm, $groupmode, $currentgroup, $modcontext);
    ...
    $sql = "SELECT $postdata, d.id as discussionid, d.name as discussionname, d.timemodified, d.usermodified, d.groupid,
             ...
             WHERE d.twf = {$twf->id} AND p.parent = 0
...
/**
 * Generates the SQL query used to get the Post details from the twf table of the database
 *
 * @param stdClass $twf     the twf object
 * @param stdClass $cm        Course Module object
 * @param int      $newsince  check for items since this epoch timestamp
 * @return string the SQL query to be used to get the Post details from the twf table of the database
 */
function twf_rss_feed_posts_sql($twf, $cm, $newsince=0) {
    $modcontext = context_module::instance($cm->id);
...
/**
 * Retrieve the correct SQL snippet for group-only twfs
 *
 * @param stdClass $cm           Course Module object
 * @param int      $groupmode    the mode in which the twf's groups are operating
 * @param bool     $currentgroup true if the user is from the a group enabled on the twf
 * @param stdClass $modcontext   The context instance of the twf module
 * @return string SQL Query for group details of the twf
 */
function twf_rss_get_group_sql($cm, $groupmode, $currentgroup, $modcontext=null) {
    $groupselect = '';
    $params = array();
...
function twf_rss_feed_contents($twf, $sql, $params, $context) {
    ...
    foreach ($recs as $rec) {
            ...
            } else if (!$isdiscussion && !twf_user_can_see_post($twf, $discussion, $post, $USER, $cm)) {
    ...
    $header = rss_standard_header(strip_tags(format_string($twf->name,true)),
                                  $CFG->wwwroot."/mod/twf/view.php?f=".$twf->id,
                                  format_string($twf->intro,true)); // TODO: fix format
