 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_twf_events_testcase extends advanced_testcase {
    ...
    public function test_course_searched_searchterm_validation() {
        ...
        \mod_twf\event\course_searched::create($params);
    }
    ...
    public function test_course_searched() {
        ...
        $this->assertInstanceOf('\mod_twf\event\course_searched', $event);
        $this->assertEquals($coursectx, $event->get_context());
        $expected = array($course->id, 'twf', 'search', "search.php?id={$course->id}&amp;search={$searchterm}", $searchterm);
    ...
    /**
     * Ensure discussion_created event validates that twfid is set.
     */
    public function test_discussion_created_twfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $context = context_module::instance($twf->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\discussion_created::create($params);
    }
    ...
    public function test_discussion_created() {
        ...
        $this->assertInstanceOf('\mod_twf\event\discussion_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'add discussion', "discuss.php?d={$discussion->id}", $discussion->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_updated event validates that twfid is set.
     */
    public function test_discussion_updated_twfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $context = context_module::instance($twf->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\discussion_updated::create($params);
    }
    ...
    public function test_discussion_updated() {
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_twf\event\discussion_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_deleted event validates that twfid is set.
     */
    public function test_discussion_deleted_twfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $context = context_module::instance($twf->cmid);
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\discussion_deleted::create($params);
    }
    ...
    public function test_discussion_deleted() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_twf\event\discussion_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'delete discussion', "view.php?id={$twf->cmid}", $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure discussion_moved event validates that fromtwfid is set.
     */
    public function test_discussion_moved_fromtwfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        ...
        $params = array(
            'context' => $context,
            'other' => array('totwfid' => $totwf->id)
        );

        $this->setExpectedException('coding_exception', 'The \'fromtwfid\' value must be set in other.');
        \mod_twf\event\discussion_moved::create($params);
    }

    /**
     * Ensure discussion_moved event validates that totwfid is set.
     */
    public function test_discussion_moved_totwfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        ...
        $params = array(
            'context' => $context,
            'other' => array('fromtwfid' => $fromtwf->id)
        );

        $this->setExpectedException('coding_exception', 'The \'totwfid\' value must be set in other.');
        \mod_twf\event\discussion_moved::create($params);
    }
    ...
    public function test_discussion_moved_context_validation() {
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $discussion->id,
            'other' => array('fromtwfid' => $fromtwf->id, 'totwfid' => $totwf->id)
        );
    ...
    public function test_discussion_moved() {
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'move discussion', "discuss.php?d={$discussion->id}",
            $discussion->id, $totwf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_discussion_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_twf\event\discussion_viewed::create($params);
    }
    ...
    public function test_course_module_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_twf\event\course_module_viewed::create($params);
    }
    ...
    public function test_course_module_viewed() {
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'view twf', "view.php?f={$twf->id}", $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
    ...
    /**
     * Ensure subscription_created event validates that the twfid is set.
     */
    public function test_subscription_created_twfid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\subscription_created::create($params);
    }
    ...
    public function test_subscription_created() {
        ...
        $record['twf'] = $twf->id;
        $record['userid'] = $user->id;
        $subscription = $this->getDataGenerator()->get_plugin_generator('mod_twf')->create_subscription($record);
        ...
        $event = \mod_twf\event\subscription_created::create($params);
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'subscribe', "view.php?f={$twf->id}", $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/subscribers.php', array('id' => $twf->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure subscription_deleted event validates that the twfid is set.
     */
    public function test_subscription_deleted_twfid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\subscription_deleted::create($params);
    }
    ...
    public function test_subscription_deleted() {
        ...
        $event = \mod_twf\event\subscription_deleted::create($params);
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'unsubscribe', "view.php?f={$twf->id}", $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/subscribers.php', array('id' => $twf->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     * Ensure readtracking_enabled event validates that the twfid is set.
     */
    public function test_readtracking_enabled_twfid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\readtracking_enabled::create($params);
    }
    ...
    public function test_readtracking_enabled() {
        ...
        $event = \mod_twf\event\readtracking_enabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_twf\event\readtracking_enabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'start tracking', "view.php?f={$twf->id}", $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/view.php', array('f' => $twf->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     *  Ensure readtracking_disabled event validates that the twfid is set.
     */
    public function test_readtracking_disabled_twfid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\readtracking_disabled::create($params);
    }
    ...
    public function test_readtracking_disabled() {
        ...
        $event = \mod_twf\event\readtracking_disabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_twf\event\readtracking_disabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'stop tracking', "view.php?f={$twf->id}", $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/view.php', array('f' => $twf->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    /**
     *  Ensure subscribers_viewed event validates that the twfid is set.
     */
    public function test_subscribers_viewed_twfid_validation() {
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        ...
        $this->setExpectedException('coding_exception', 'The \'twfid\' value must be set in other.');
        \mod_twf\event\subscribers_viewed::create($params);
    }
    ...
    public function test_user_report_viewed_reportmode_validation() {
        ...
        \mod_twf\event\user_report_viewed::create($params);
    }
    ...
    public function test_user_report_viewed() {
        ...
        $event = \mod_twf\event\user_report_viewed::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_twf\event\user_report_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'user report',
    ...
    public function test_post_created_postid_validation() {
        ...
        $params = array(
            'context' => context_module::instance($twf->cmid),
            'other' => array('twfid' => $twf->id, 'twftype' => $twf->type, 'discussionid' => $discussion->id)
        );
    ...
    public function test_post_created_discussionid_validation() {
        ...
        $params = array(
            'context' => context_module::instance($twf->cmid),
            'objectid' => $post->id,
            'other' => array('twfid' => $twf->id, 'twftype' => $twf->type)
        );
    ...
    /**
     *  Ensure post_created event validates that the twfid is set.
     */
    public function test_post_created_twfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
        ...
        $params = array(
            'context' => context_module::instance($twf->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'twftype' => $twf->type)
        );
    ...
    /**
     *  Ensure post_created event validates that the twftype is set.
     */
    public function test_post_created_twftype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
        ...
        $params = array(
            'context' => context_module::instance($twf->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'twfid' => $twf->id)
        );

        $this->setExpectedException('coding_exception', 'The \'twftype\' value must be set in other.');
        \mod_twf\event\post_created::create($params);
    }
    ...
    public function test_post_created_context_validation() {
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'twfid' => $twf->id, 'twftype' => $twf->type)
        );
    ...
    public function test_post_created() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_twf\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'add post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/discuss.php', array('d' => $discussion->id));
    ...
    /**
     * Test the post_created event for a single discussion twf.
     */
    public function test_post_created_single() {
        ...
        $this->assertInstanceOf('\mod_twf\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'add post', "view.php?f={$twf->id}#p{$post->id}",
            $twf->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/view.php', array('f' => $twf->id));
        $url->set_anchor('p'.$event->objectid);
    ...
    public function test_post_deleted_postid_validation() {
        ...
        \mod_twf\event\post_deleted::create($params);
    }
    ...
    /**
     *  Ensure post_deleted event validates that the twfid is set.
     */
    public function test_post_deleted_twfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    /**
     *  Ensure post_deleted event validates that the twftype is set.
     */
    public function test_post_deleted_twftype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    public function test_post_deleted() {
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'delete post', "discuss.php?d={$discussion->id}", $post->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/discuss.php', array('d' => $discussion->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_post_updated_discussionid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'discussionid\' value must be set in other.');
        \mod_twf\event\post_updated::create($params);
    }

    /**
     *  Ensure post_updated event validates that the twfid is set.
     */
    public function test_post_updated_twfid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    /**
     *  Ensure post_updated event validates that the twftype is set.
     */
    public function test_post_updated_twftype_validation() {
        $course = $this->getDataGenerator()->create_course();
        $twf = $this->getDataGenerator()->create_module('twf', array('course' => $course->id));
        $user = $this->getDataGenerator()->create_user();
    ...
    public function test_post_updated() {
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'update post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $post->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/discuss.php', array('d' => $discussion->id));
        $url->set_anchor('p'.$event->objectid);
    ...
    public function test_post_updated_single() {
        ...
        $this->assertInstanceOf('\mod_twf\event\post_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'twf', 'update post', "view.php?f={$twf->id}#p{$post->id}",
            $post->id, $twf->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/twf/view.php', array('f' => $twf->id));
    ...
    public function test_discussion_subscription_created() {
        ...
        // Trigger the event by subscribing the user to the twf discussion.
    ...
    public function test_discussion_subscription_created_validation() {
        ...
        // The user is not subscribed to the twf. Insert a new discussion subscription.
        ...
        $event = \mod_twf\event\discussion_subscription_created::create($params);
    ...
    public function test_discussion_subscription_created_validation_contextlevel() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_twf\event\discussion_subscription_created::create($params);
    }
    ...
    /**
     * Test twfid validation of discussion_subscription_created event.
     */
    public function test_discussion_subscription_created_validation_twfid() {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/mod/twf/lib.php');
    ...
    public function test_discussion_subscription_deleted() {
        ...
        // Trigger the event by unsubscribing the user to the twf discussion.
    ...
    public function test_discussion_subscription_deleted_validation() {
        ...
        $subscription->twf = $twf->id;
        $subscription->discussion = $discussion->id;
        $subscription->preference = \mod_twf\subscriptions::FORUM_DISCUSSION_UNSUBSCRIBED;
        ...
        $event = \mod_twf\event\discussion_subscription_deleted::create($params);
    ...
    public function test_discussion_subscription_deleted_validation_contextlevel() {
        ...
        // Without an invalid context.
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_twf\event\discussion_subscription_deleted::create($params);
    }
    ...
    /**
     * Test twfid validation of discussion_subscription_deleted event.
     */
    public function test_discussion_subscription_deleted_validation_twfid() {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/mod/twf/lib.php');
