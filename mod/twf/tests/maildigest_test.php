...
class mod_twf_maildigest_testcase extends advanced_testcase {
    ...
    /**
     * Setup a user, course, and twfs.
     *
     * @return stdClass containing the list of twfs, courses, twfids,
    ...
    protected function helper_setup_user_in_course() {
        ...
        $return->twfs->twf1 = $this->getDataGenerator()->create_module('twf', $record);
        $return->twfsids[] = $return->twfs->twf1->id;

        $return->twfs->twf2 = $this->getDataGenerator()->create_module('twf', $record);
        $return->twfsids[] = $return->twfs->twf2->id;

        // Check the twf was correctly created.
        list ($test, $params) = $DB->get_in_or_equal($return->twfsids);
    ...
    /**
     * Helper to falsify all twf post records for a digest run.
     */
    protected function helper_force_digest_mail_times() {
        ...
        $DB->set_field('twf_posts', 'modified', $digesttime, array('mailed' => 0));
        $DB->set_field('twf_posts', 'created', $digesttime, array('mailed' => 0));
    }
    ...
    public function test_set_maildigest() {
        ...
        $currentsetting = $DB->get_record('twf_digests', array(
            'twf' => $twf1->id,
            'userid' => $user->id,
        ));
        ...
        twf_set_user_maildigest($twf1, 0, $user);
        $currentsetting = $DB->get_record('twf_digests', array(
            'twf' => $twf1->id,
            'userid' => $user->id,
        ));
        ...
        twf_set_user_maildigest($twf1, 2, $user);
        $currentsetting = $DB->get_record('twf_digests', array(
            'twf' => $twf1->id,
            'userid' => $user->id,
        ));
        ...
        twf_set_user_maildigest($twf1, 42, $user);
    }
    ...
    public function test_get_user_digest_options_default() {
        ...
        $digestoptions = array(
            '0' => get_string('emaildigestoffshort', 'mod_twf'),
            '1' => get_string('emaildigestcompleteshort', 'mod_twf'),
            '2' => get_string('emaildigestsubjectsshort', 'mod_twf'),
        );

        // The default settings is 0.
        $this->assertEquals(0, $user->maildigest);
        $options = twf_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_twf', $digestoptions[0]));

        // Update the setting to 1.
        $USER->maildigest = 1;
        $this->assertEquals(1, $USER->maildigest);
        $options = twf_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_twf', $digestoptions[1]));

        // Update the setting to 2.
        $USER->maildigest = 2;
        $this->assertEquals(2, $USER->maildigest);
        $options = twf_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_twf', $digestoptions[2]));
    }
    ...
    public function test_cron_no_posts() {
        ...
        // Initially the twf cron should generate no messages as we've made no posts.
    ...
    public function test_cron_profile_single_mails() {
        ...
        // Add some discussions to the twfs.
        $record = new stdClass();
        ...
        // Set the maildigest preference for twf1 to default.
        twf_set_user_maildigest($twf1, -1, $user);

        // Set the maildigest preference for twf2 to default.
        twf_set_user_maildigest($twf2, -1, $user);

        // No digests mails should be sent, but 10 twf mails will be sent.
    ...
    /**
     * Sends several notifications to one user as:
     * * daily digests coming from the per-twf setting; and
    ...
    public function test_cron_mixed_email_1() {
        ...
        // Set the maildigest preference for twf1 to digest.
        twf_set_user_maildigest($twf1, 1, $user);

        // Set the maildigest preference for twf2 to default (single).
        twf_set_user_maildigest($twf2, -1, $user);
    ...
    /**
     * Sends several notifications to one user as:
     * * single e-mails from the per-twf setting; and
    ...
    public function test_cron_mixed_email_2() {
        ...
        // Set the maildigest preference for twf2 to single.
        twf_set_user_maildigest($twf2, 0, $user);
