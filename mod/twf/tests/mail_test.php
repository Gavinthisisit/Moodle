...
/**
 * The twf module mail generation tests.
 *
 * @package    mod_twf
 * @category   external
...
class mod_twf_mail_testcase extends advanced_testcase {
    ...
    /**
     * Perform message inbound setup for the mod_twf reply handler.
     */
    protected function helper_spoof_message_inbound_setup() {
        ...
        $record = $DB->get_record('messageinbound_handlers', array('classname' => '\mod_twf\message\inbound\reply_handler'));
        $record->enabled = true;
     ...
     */
    protected function helper_post_to_twf($twf, $author, $fields = array()) {
        global $DB;
    ...
    protected function helper_update_post_time($post, $factor) {
        ...
        $DB->set_field('twf_posts', 'created', $post->created + $factor, array('id' => $post->id));
    }
    ...
    protected function helper_update_subscription_time($user, $discussion, $factor) {
        ...
        $sub = $DB->get_record('twf_discussion_subs', array('userid' => $user->id, 'discussion' => $discussion->id));

        // Update the subscription to have a preference in the past.
        $DB->set_field('twf_discussion_subs', 'preference', $sub->preference + $factor, array('id' => $sub->id));
    }
     ...
     * @param stdClass $author The author to post as
     * @return stdClass The twf post
     */
    protected function helper_post_to_discussion($twf, $discussion, $author) {
        global $DB;
     ...
     *
     * @param stdClass $post The twf post object
    ...
    /**
     * Run the twf cron, and check that the specified posts were sent the
     * specified number of times.
     *
     * @param stdClass $post The twf post object
     * @param integer $expected The number of times that the post should have been sent
    ...
    public function test_subscription_disabled() {
        ...
        $this->assertEquals($expected, $DB->count_records('twf_subscriptions', array(
            'userid'        => $recipient->id,
            'twf'         => $twf->id,
        )));

        // Run cron and check that the expected number of users received the notification.
        list($discussion, $post) = $this->helper_post_to_twf($twf, $recipient);
        $messages = $this->helper_run_cron_check_count($post, $expected);

        // Unsubscribe the user again.
        \mod_twf\subscriptions::unsubscribe_user($recipient->id, $twf);
        ...
        \mod_twf\subscriptions::subscribe_user_to_discussion($recipient->id, $discussion);
    ...
    public function test_optional_with_subscribed_user() {
        ...
        // Subscribe the 'recipient' user from the twf.
        \mod_twf\subscriptions::subscribe_user($recipient->id, $twf);
    ...
    /**
     * Test that a user unsubscribed from a twf who has subscribed to a discussion, only receives posts made after
     * they subscribed to the discussion.
     */
    public function test_twf_discussion_subscription_twf_unsubscribed_discussion_subscribed_after_post() {
        $this->resetAfterTest(true);
    ...
    public function test_long_subject() {
        ...
        // Post a discussion to the twf.
        $subject = 'This is the very long twf post subject that somebody was very kind of leaving, it is intended to check if long subject comes in mail correctly. Thank you.';
        $a = (object)array('courseshortname' => $course->shortname, 'twfname' => $twf->name, 'subject' => $subject);
        $expectedsubject = get_string('postmailsubject', 'twf', $a);
        list($discussion, $post) = $this->helper_post_to_twf($twf, $author, array('name' => $subject));
