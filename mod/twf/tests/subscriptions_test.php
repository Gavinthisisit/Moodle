...
class mod_twf_subscriptions_testcase extends advanced_testcase {
    ...
    /**
     * Create a new discussion and post within the specified twf, as the
     * specified author.
     *
     * @param stdClass $twf The twf to post in
     * @param stdClass $author The author to post as
     * @param array An array containing the discussion object, and the post object
     */
    protected function helper_post_to_twf($twf, $author) {
        global $DB;
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_twf');

        // Create a discussion in the twf, and then add a post to that discussion.
        $record = new stdClass();
    ...
    /**
     * Test fetching unsubscribable twfs.
     */
    public function test_unsubscribable_twfs() {
        global $DB;
        ...
        $result = \mod_twf\subscriptions::get_unsubscribable_twfs();
        $this->assertEquals(0, count($result));

        // Create the twfs.
        $options = array('course' => $course->id, 'forcesubscribe' => FORUM_FORCESUBSCRIBE);
        $forcetwf = $this->getDataGenerator()->create_module('twf', $options);
        ...
        // At present the user is only subscribed to the initial twf.
        $result = \mod_twf\subscriptions::get_unsubscribable_twfs();
        $this->assertEquals(1, count($result));

        // Ensure that the user is enrolled in all of the twfs except force subscribed.
        \mod_twf\subscriptions::subscribe_user($user->id, $disallowtwf);
        \mod_twf\subscriptions::subscribe_user($user->id, $choosetwf);

        $result = \mod_twf\subscriptions::get_unsubscribable_twfs();
        $this->assertEquals(3, count($result));

        // Hide the twfs.
        set_coursemodule_visible($forcetwf->cmid, 0);
        set_coursemodule_visible($disallowtwf->cmid, 0);
        set_coursemodule_visible($choosetwf->cmid, 0);
        set_coursemodule_visible($initialtwf->cmid, 0);
        $result = \mod_twf\subscriptions::get_unsubscribable_twfs();
        $this->assertEquals(0, count($result));
        ...
        // All of the unsubscribable twfs should now be listed.
        $result = \mod_twf\subscriptions::get_unsubscribable_twfs();
        $this->assertEquals(3, count($result));
    }

    /**
     * Test that toggling the twf-level subscription for a different user does not affect their discussion-level
     * subscriptions.
     */
    public function test_twf_subscribe_toggle_as_other() {
        global $DB;
        ...
        // Check that the user is currently not subscribed to the twf.
        $this->assertFalse(\mod_twf\subscriptions::is_subscribed($author->id, $twf));
        ...
        $this->assertEquals(0, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
        )));
        $this->assertEquals(0, $DB->count_records('twf_discussion_subs', array(
            'userid'        => $author->id,
        ...
        $this->assertEquals(1, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
        )));
        $this->assertEquals(0, $DB->count_records('twf_discussion_subs', array(
        ...
        // Unsubscribing should remove the record from the twf subscriptions table, and not modify the twf
        ...
        // Subscribing to the twf should create a record in the subscriptions table, but not the twf discussion
        // subscriptions table.
        twf_subscribe($author->id, $twf->id);
        $this->assertEquals(1, count(phpunit_util::get_debugging_messages()));
        phpunit_util::reset_debugging();
        $this->assertEquals(1, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
        ...
        // Unsubscribing should remove the record from the twf subscriptions table, and not modify the twf
        // discussion subscriptions table.
        twf_unsubscribe($author->id, $twf->id);
        $this->assertEquals(1, count(phpunit_util::get_debugging_messages()));
        phpunit_util::reset_debugging();
        $this->assertEquals(0, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
        ...
        // Enroling the user in the discussion should add one record to the twf discussion table without modifying the
        // form subscriptions.
        ...
        $this->assertEquals(1, $DB->count_records('twf_discussion_subs', array(
            'userid'        => $author->id,
            'discussion'    => $discussion->id,
        )));
        ...
        // Re-subscribe to the discussion so that we can check the effect of twf-level subscriptions.
        \mod_twf\subscriptions::subscribe_user_to_discussion($author->id, $discussion);
        $this->assertEquals(0, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
        ...
        // Subscribing to the twf should have no effect on the twf discussion subscriptions table if the user did
        ...
        // Unsubscribing from the twf should have no effect on the twf discussion subscriptions table if the user
        ...
        // Subscribing to the twf should remove the per-discussion subscription preference if the user requested the
        // change themself.
        \mod_twf\subscriptions::subscribe_user($author->id, $twf, null, true);
        $this->assertEquals(1, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
        )));
        ...
        // Now unsubscribe from the current discussion whilst being subscribed to the twf as a whole.
        \mod_twf\subscriptions::unsubscribe_user_from_discussion($author->id, $discussion);
        $this->assertEquals(1, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
        ...
        // Unsubscribing from the twf should remove the per-discussion subscription preference if the user requested the
        // change themself.
        \mod_twf\subscriptions::unsubscribe_user($author->id, $twf, null, true);
        $this->assertEquals(0, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
        )));
        ...
        // Subscribe to the twf without removing the discussion preferences.
        \mod_twf\subscriptions::subscribe_user($author->id, $twf);
        $this->assertEquals(1, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
    ...
    /**
     * Test that a user unsubscribed from a twf is not subscribed to it's discussions by default.
     */
    public function test_twf_discussion_subscription_twf_unsubscribed() {
        $this->resetAfterTest(true);
    ...
    /**
     * Test that the act of subscribing to a twf subscribes the user to it's discussions by default.
     */
    public function test_twf_discussion_subscription_twf_subscribed() {
        $this->resetAfterTest(true);
        ...
        // Enrol the user in the twf.
        // If a subscription was added, we get the record ID.
        $this->assertInternalType('int', \mod_twf\subscriptions::subscribe_user($author->id, $twf));

        // If we already have a subscription when subscribing the user, we get a boolean (true).
        $this->assertTrue(\mod_twf\subscriptions::subscribe_user($author->id, $twf));

        // Check that the user is currently subscribed to the twf.
        $this->assertTrue(\mod_twf\subscriptions::is_subscribed($author->id, $twf));
        ...
        $this->assertTrue(\mod_twf\subscriptions::is_subscribed($author->id, $twf, $discussion->id));
    }

    /**
     * Test that a user unsubscribed from a twf can be subscribed to a discussion.
     */
    public function test_twf_discussion_subscription_twf_unsubscribed_discussion_subscribed() {
        $this->resetAfterTest(true);
        ...
        $this->assertFalse(\mod_twf\subscriptions::unsubscribe_user_from_discussion($author->id, $discussion));

        // Then subscribe them to the discussion.
        $this->assertTrue(\mod_twf\subscriptions::subscribe_user_to_discussion($author->id, $discussion));

        // Check that the user is still unsubscribed from the twf.
        $this->assertFalse(\mod_twf\subscriptions::is_subscribed($author->id, $twf));
    ...
    /**
     * Test that a user subscribed to a twf can be unsubscribed from a discussion.
     */
    public function test_twf_discussion_subscription_twf_subscribed_discussion_unsubscribed() {
        $this->resetAfterTest(true);
        ...
        // Check that the user is still subscribed to the twf.
        $this->assertTrue(\mod_twf\subscriptions::is_subscribed($author->id, $twf));
    ...
    /**
     * Test the effect of toggling the discussion subscription status when subscribed to the twf.
     */
    public function test_twf_discussion_toggle_twf_subscribed() {
        global $DB;
        ...
        $this->assertFalse(\mod_twf\subscriptions::subscribe_user_to_discussion($author->id, $discussion));

        // And there should be no discussion subscriptions (and one twf subscription).
        $this->assertEquals(0, $DB->count_records('twf_discussion_subs', array(
            'userid'        => $author->id,
            'discussion'    => $discussion->id,
        )));
        ...
        // And there should be a discussion subscriptions (and one twf subscription).
        $this->assertEquals(1, $DB->count_records('twf_discussion_subs', array(
            'userid'        => $author->id,
            'discussion'    => $discussion->id,
        )));
        ...
        // And one in the twf subscription tracking table.
        $this->assertEquals(1, $DB->count_records('twf_subscriptions', array(
            'userid'        => $author->id,
            'twf'         => $twf->id,
        )));
        ...
        // Now unsubscribe the user from the twf.
        $this->assertTrue(\mod_twf\subscriptions::unsubscribe_user($author->id, $twf, null, true));

        // This removes both the twf_subscriptions, and the twf_discussion_subs records.
        $this->assertEquals(0, $DB->count_records('twf_discussion_subs', array(
            'userid'        => $author->id,
            'discussion'    => $discussion->id,
        )));
        ...
        $result = \mod_twf\subscriptions::fetch_discussion_subscription($twf->id, $author->id);
        $this->assertInternalType('array', $result);
        $this->assertFalse(isset($result[$discussion->id]));
    }

    /**
     * Test the effect of toggling the discussion subscription status when unsubscribed from the twf.
     */
    public function test_twf_discussion_toggle_twf_unsubscribed() {
        global $DB;
    ...
    /**
     * Test that the correct users are returned when fetching subscribed users from a twf where users can choose to
    ...
    public function test_fetch_subscribed_users_subscriptions() {
        ...
        // Create a course, with a twf. where users are initially subscribed.
        $course = $this->getDataGenerator()->create_course();
        $options = array('course' => $course->id, 'forcesubscribe' => FORUM_INITIALSUBSCRIBE);
        $twf = $this->getDataGenerator()->create_module('twf', $options);
        ...
        $subscribers = \mod_twf\subscriptions::fetch_subscribed_users($twf);
        $this->assertEquals($usercount, count($subscribers));

        // Subscribe the guest user too to the twf - they should never be returned by this function.
        ...
        for ($i = 0; $i < $unsubscribedcount; $i++) {
            \mod_twf\subscriptions::unsubscribe_user($users[$i]->id, $twf);
        }
    ...
    /**
     * Test that the correct users are returned hwen fetching subscribed users from a twf where users are forcibly
     * subscribed.
     */
    ...
    public function test_fetch_subscribed_users_discussion_subscriptions() {
        ...
        list($discussion, $post) = $this->helper_post_to_twf($twf, $users[0]);

        // All users should be subscribed.
        $subscribers = \mod_twf\subscriptions::fetch_subscribed_users($twf);
        $this->assertEquals($usercount, count($subscribers));
        $subscribers = \mod_twf\subscriptions::fetch_subscribed_users($twf, 0, null, null, true);
        $this->assertEquals($usercount, count($subscribers));

        \mod_twf\subscriptions::unsubscribe_user_from_discussion($users[0]->id, $discussion);
        ...
        $DB->insert_record('twf_discussion_subs', $record);
        ...
        for ($i = 0; $i < $subscribeddiscussionusers; $i++) {
            \mod_twf\subscriptions::subscribe_user_to_discussion($users[$i]->id, $discussion);
        }
        $subscribers = \mod_twf\subscriptions::fetch_subscribed_users($twf);
        $this->assertEquals($usercount - $unsubscribedcount, count($subscribers));
        $subscribers = \mod_twf\subscriptions::fetch_subscribed_users($twf, 0, null, null, true);
    ...
    /**
     * Test whether a user is force-subscribed to a twf.
     */
    public function test_force_subscribed_to_twf() {
        global $DB;
        ...
        assign_capability('mod/twf:allowforcesubscribe', CAP_PROHIBIT, $roleids['student'], $context);
        $context->mark_dirty();
        $this->assertFalse(has_capability('mod/twf:allowforcesubscribe', $context, $user->id));

        // Check that the user is no longer subscribed to the twf.
    ...
    public function test_subscription_cache_prefill() {
        ...
        $this->assertNull(\mod_twf\subscriptions::fill_subscription_cache($twf->id));
        ...
        foreach ($users as $user) {
            $this->assertTrue(\mod_twf\subscriptions::fetch_subscription_cache($twf->id, $user->id));
        }
    ...
    public function test_subscription_cache_fill() {
        ...
        // Fetch some subscriptions from that twf - these should not use the cache and will perform additional queries.
        foreach ($users as $user) {
            $this->assertTrue(\mod_twf\subscriptions::fetch_subscription_cache($twf->id, $user->id));
        }
        $finalcount = $DB->perf_get_reads();
    ...
    public function test_discussion_subscription_cache_prefill() {
        ...
        $startcount = $DB->perf_get_reads();
        $this->assertNull(\mod_twf\subscriptions::fill_discussion_subscription_cache($twf->id));
        $postfillcount = $DB->perf_get_reads();
        $this->assertEquals(1, $postfillcount - $startcount);
