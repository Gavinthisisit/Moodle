 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_twf_generator extends testing_module_generator {

    /**
     * @var int keep track of how many twf discussions have been created.
     */
    protected $twfdiscussioncount = 0;

    /**
     * @var int keep track of how many twf posts have been created.
     */
    protected $twfpostcount = 0;

    /**
     * @var int keep track of how many twf subscriptions have been created.
     */
    protected $twfsubscriptionscount = 0;
    ...
    public function reset() {
        $this->twfdiscussioncount = 0;
        $this->twfpostcount = 0;
        $this->twfsubscriptionscount = 0;
    ...
    public function create_subscription($record = null) {
        ...
        // Increment the twf subscription count.
        $this->twfsubscriptionscount++;
        ...
        if (!isset($record['twf'])) {
            throw new coding_exception('twf must be present in phpunit_util::create_subscription() $record');
        }
        ...
        $record->id = $DB->insert_record('twf_subscriptions', $record);
    ...
    public function create_content($instance, $record = array()) {
        ...
        if (empty($record['discussion']) && empty($record['parent'])) {
            ...
            $post = $DB->get_record('twf_posts', array('id' => $discussion->firstpost));
        } else {
            // Create post.
            if (empty($record['parent'])) {
                $record['parent'] = $DB->get_field('twf_discussions', 'firstpost', array('id' => $record['discussion']), MUST_EXIST);
            } else if (empty($record['discussion'])) {
                $record['discussion'] = $DB->get_field('twf_posts', 'discussion', array('id' => $record['parent']), MUST_EXIST);
            }
