...
/**
 * This file contains a custom renderer class used by the twf module.
 *
 * @package   mod_twf
 ...
 * is used by the twf module.
 *
 * @package   mod_twf
 * @copyright 2009 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 **/
class mod_twf_renderer extends plugin_renderer_base {
    ...
    public function neighbouring_discussion_navigation($prev, $next) {
        ...
        if ($prev || $next) {
            ...
            if ($prev) {
                $url = new moodle_url('/mod/twf/discuss.php', array('d' => $prev->id));
                $html .= html_writer::start_tag('li', array('class' => 'prev-discussion'));
                $html .= html_writer::link($url, format_string($prev->name),
                    array('aria-label' => get_string('prevdiscussiona', 'mod_twf', format_string($prev->name))));
                $html .= html_writer::end_tag('li');
            }
            if ($next) {
                $url = new moodle_url('/mod/twf/discuss.php', array('d' => $next->id));
                $html .= html_writer::start_tag('li', array('class' => 'next-discussion'));
                $html .= html_writer::link($url, format_string($next->name),
                    array('aria-label' => get_string('nextdiscussiona', 'mod_twf', format_string($next->name))));
                $html .= html_writer::end_tag('li');
            }
     ...
     * @return string
     */
    public function subscriber_overview($users, $twf , $course) {
        $output = '';
        $modinfo = get_fast_modinfo($course);
        if (!$users || !is_array($users) || count($users)===0) {
            $output .= $this->output->heading(get_string("nosubscribers", "twf"));
        } else if (!isset($modinfo->instances['twf'][$twf->id])) {
            ...
            $output .= $this->output->heading(get_string("subscribersto","twf", "'".format_string($twf->name)."'"));
            $table = new html_table();
            $table->cellpadding = 5;
            $table->cellspacing = 5;
    ...
    public function subscribed_users(user_selector_base $existingusers) {
        ...
        $output .= html_writer::tag('p', get_string('forcesubscribed', 'twf'));
