...
/**
 * File containing the form definition to post in the twf.
 *
 * @package   mod_twf
...
/**
 * Class to post in a twf.
 *
 * @package   mod_twf
 * @copyright Jamie Pratt <me@jamiep.org>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_twf_post_form extends moodleform {

    /**
     * Returns the options array to use in filemanager for twf attachments
     *
     * @param stdClass $twf
     * @return array
     */
    public static function attachment_options($twf) {
        global $COURSE, $PAGE, $CFG;
        $maxbytes = get_user_max_upload_file_size($PAGE->context, $CFG->maxbytes, $COURSE->maxbytes, $twf->maxbytes);
        return array(
            'subdirs' => 0,
            'maxbytes' => $maxbytes,
            'maxfiles' => $twf->maxattachments,
            'accepted_types' => '*',
    ...
    public static function editor_options(context_module $context, $postid) {
        ...
        return array(
            ...
            'subdirs' => file_area_contains_subdirs($context, 'mod_twf', 'post', $postid)
        );
    }
    ...
    function definition() {
        ...
        } else if (\mod_twf\subscriptions::subscription_disabled($twf) && !$manageactivities) {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'twf'));
            $mform->freeze('discussionsubscribe');
            $mform->setDefaults('discussionsubscribe', 0);
        ...
        if (!empty($twf->maxattachments) && $twf->maxbytes != 1 && has_capability('mod/twf:createattachment', $modcontext))  {  //  1 = No attachments at all
        ...
        if (!empty($CFG->twf_enabletimedposts) && !$post->parent && has_capability('mod/twf:viewhiddentimedposts', $coursecontext)) { // hack alert
    ...
    function validation($data, $files) {
        ...
        if (($data['timeend']!=0) && ($data['timestart']!=0) && $data['timeend'] <= $data['timestart']) {
            $errors['timeend'] = get_string('timestartenderror', 'twf');
        }
        if (empty($data['message']['text'])) {
            $errors['message'] = get_string('erroremptymessage', 'twf');
        }
        if (empty($data['subject'])) {
            $errors['subject'] = get_string('erroremptysubject', 'twf');
        }
        return $errors;
    }
}
