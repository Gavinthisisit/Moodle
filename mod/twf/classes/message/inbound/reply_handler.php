...
/**
 * A Handler to process replies to twf posts.
 *
 * @package    mod_twf
...
namespace mod_twf\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'mod_twf');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'mod_twf');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (empty($twf->maxattachments) || $twf->maxbytes == 1 ||
                    !has_capability('mod/twf:createattachment', $modcontext)) {
            ...
            if ($twf->maxattachments < $attachmentcount) {
                ...
                mtrace("--> User attached {$attachmentcount} files when only {$twf->maxattachments} where allowed. "
                     . " Rejecting e-mail.");
            ...
            if ($twf->maxbytes < $filesize) {
                // Too many attachments.
                mtrace("--> User attached {$filesize} bytes of files when only {$twf->maxbytes} where allowed. "
                     . "Rejecting e-mail.");
                $data = new \stdClass();
                $data->twf = $twf;
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/mod/twf/discuss.php', array('d' => $handlerresult->discussion));
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'mod_twf', $a);
        $message->html = get_string('postbymailsuccess_html', 'mod_twf', $a);
        return $message;
    }
}
