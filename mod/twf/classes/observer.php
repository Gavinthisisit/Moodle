...
/**
 * Event observer for mod_twf.
 */
class mod_twf_observer {
    ...
    public static function user_enrolment_deleted(\core\event\user_enrolment_deleted $event) {
        ...
        if ($cp->lastenrol) {
            ...
            $twfselect = "IN (SELECT f.id FROM {twf} f WHERE f.course = :courseid)";

            $DB->delete_records_select('twf_digests', 'userid = :userid AND twf '.$twfselect, $params);
            $DB->delete_records_select('twf_subscriptions', 'userid = :userid AND twf '.$twfselect, $params);
            $DB->delete_records_select('twf_track_prefs', 'userid = :userid AND twfid '.$twfselect, $params);
            $DB->delete_records_select('twf_read', 'userid = :userid AND twfid '.$twfselect, $params);
        }
    }
    ...
    public static function course_module_created(\core\event\course_module_created $event) {
        ...
        if ($event->other['modulename'] === 'twf') {
            // Include the twf library to make use of the twf_instance_created function.
            require_once($CFG->dirroot . '/mod/twf/lib.php');

            $twf = $event->get_record_snapshot('twf', $event->other['instanceid']);
            twf_instance_created($event->get_context(), $twf);
        }
    }
}
