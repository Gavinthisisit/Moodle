...
/**
 * Prints the list of all teamworks in the course
 *
...
foreach ($teamworks as $teamwork) {
    if (empty($teamwork->visible)) {
        $link = html_writer::link(new moodle_url('/mod/teamwork/view.php', array('id' => $teamwork->coursemodule)),
                                  $teamwork->name, array('class' => 'dimmed'));
    } else {
        $link = html_writer::link(new moodle_url('/mod/teamwork/view.php', array('id' => $teamwork->coursemodule)),
                                  $teamwork->name);
    }
    ...
    if ($usesections) {
        $table->data[] = array(get_section_name($course, $teamwork->section), $link);
    } else {
        $table->data[] = array($link);
    }
}
echo $OUTPUT->heading(get_string('modulenameplural', 'teamwork'), 3);
