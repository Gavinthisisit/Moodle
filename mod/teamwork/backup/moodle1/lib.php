...
/**
 * Teamwork conversion handler
 */
class moodle1_mod_teamwork_handler extends moodle1_mod_handler {

    /** @var array the temporary in-memory cache for the current <MOD> contents */
    protected $currentteamwork = null;

    /** @var array in-memory cache for the course module information for the current teamwork  */
    protected $currentcminfo = null;

    /** @var array the mapping of legacy elementno => newelementid for the current teamwork */
    protected $newelementids = array();

    /** @var moodle1_file_manager for the current teamwork */
    protected $fileman = null;

    /** @var moodle1_inforef_manager */
    protected $inforefman = null;

    /** @var array list of moodle1_teamworkform_handler instances */
    ...
    public function get_paths() {
        return array(
            new convert_path('teamwork', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WORKSHOP'),
            new convert_path('teamwork_elements', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS'),
            new convert_path(
                'teamwork_element', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT',
                array(
                    'dropfields' => array(
                        'stddev',
                        'totalassessments',
            ...
            new convert_path('teamwork_element_rubric', '/MOODLE_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT/RUBRICS/RUBRIC'),
        );
    }
     ...
     * data available
     */
    public function process_teamwork($data, $raw) {

        // re-use the upgrade function to convert teamwork record
        $fakerecord = (object)$data;
        $fakerecord->course = 12345678;
        $this->currentteamwork = (array)teamwork_upgrade_transform_instance($fakerecord);
        unset($this->currentteamwork['course']);

        // add the new fields with the default values
        $this->currentteamwork['id']                        = $data['id'];
        $this->currentteamwork['evaluation']                = 'best';
        $this->currentteamwork['examplesmode']              = teamwork::EXAMPLES_VOLUNTARY;
        $this->currentteamwork['gradedecimals']             = 0;
        $this->currentteamwork['instructauthors']           = '';
        $this->currentteamwork['instructauthorsformat']     = FORMAT_HTML;
        $this->currentteamwork['instructreviewers']         = '';
        $this->currentteamwork['instructreviewersformat']   = FORMAT_HTML;
        $this->currentteamwork['latesubmissions']           = 0;
        $this->currentteamwork['conclusion']                = '';
        $this->currentteamwork['conclusionformat']          = FORMAT_HTML;

        foreach (array('submissionend', 'submissionstart', 'assessmentend', 'assessmentstart') as $field) {
            if (!array_key_exists($field, $this->currentteamwork)) {
                $this->currentteamwork[$field] = null;
            }
        }
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_teamwork');

        // convert course files embedded into the intro
        $this->fileman->filearea = 'intro';
        $this->fileman->itemid   = 0;
        $this->currentteamwork['intro'] = moodle1_converter::migrate_referenced_files($this->currentteamwork['intro'], $this->fileman);

        // write teamwork.xml
        $this->open_xml_writer("activities/teamwork_{$moduleid}/teamwork.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'teamwork', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('teamwork', array('id' => $instanceid));

        foreach ($this->currentteamwork as $field => $value) {
            if ($field <> 'id') {
                $this->xmlwriter->full_tag($field, $value);
            }
        }

        return $this->currentteamwork;
    }
     ...
     * will append dimensions data in {@link self::process_teamwork_element()}
     */
    public function on_teamwork_elements_start() {

        $this->xmlwriter->begin_tag('subplugin_teamworkform_'.$this->currentteamwork['strategy'].'_teamwork');

        // inform the strategy handler that a new teamwork instance is being processed
        $handler = $this->get_strategy_handler($this->currentteamwork['strategy']);
     ...
     */
    public function process_teamwork_element($data, $raw) {
     ...
     */
    public function process_teamwork_element_rubric($data, $raw) {
        if ($this->currentteamwork['strategy'] == 'rubric') {
     ...
     */
    public function on_teamwork_element_end() {
     ...
     * This is executed when the parser reaches </ELEMENTS>
     */
    public function on_teamwork_elements_end() {
        // give the strategy hanlders last chance to write what they need
        $handler = $this->get_strategy_handler($this->currentteamwork['strategy']);
        $handler->on_elements_end();

        // close the dimensions definition
        $this->xmlwriter->end_tag('subplugin_teamworkform_'.$this->currentteamwork['strategy'].'_teamwork');
     ...
     */
    public function on_teamwork_end() {
        // close teamwork.xml
        $this->xmlwriter->end_tag('teamwork');
        ...
        $moduleid = $this->currentcminfo['id'];
        $this->open_xml_writer("activities/teamwork_{$moduleid}/inforef.xml");
        $this->inforefman->write_refs($this->xmlwriter);
        $this->close_xml_writer();

        // get ready for the next instance
        $this->currentteamwork = null;
        $this->currentcminfo   = null;
        $this->newelementids   = array();
    }

    /**
     * Provides access to the current <teamwork> data
     *
     * @return array|null
     */
    public function get_current_teamwork() {
        return $this->currentteamwork;
    }
     ...
     * @return moodle1_teamworkform_handler the instance of the handler
     */
    protected function get_strategy_handler($strategy) {
        ...
        if (is_null($this->strategyhandlers)) {
            ...
            $subplugins = core_component::get_plugin_list('teamworkform');
            foreach ($subplugins as $name => $dir) {
                $handlerfile  = $dir.'/backup/moodle1/lib.php';
                $handlerclass = "moodle1_teamworkform_{$name}_handler";
                ...
                $this->log('preparing teamwork grading strategy handler', backup::LOG_DEBUG, $handlerclass);
                $this->strategyhandlers[$name] = new $handlerclass($this, $name);
                if (!$this->strategyhandlers[$name] instanceof moodle1_teamworkform_handler) {
        ...
        if (!isset($this->strategyhandlers[$strategy])) {
            throw new moodle1_convert_exception('usupported_subplugin', 'teamworkform_'.$strategy);
        }
 ...
 */
abstract class moodle1_teamworkform_handler extends moodle1_submod_handler {
    ...
    /**
     * Provides a xml_writer instance to this teamworkform handler
     *
     ...
     *
     * Gives the handler a chance to prepare for a new teamwork instance
     */
...
/**
 * Given a record containing data from 1.9 teamwork table, returns object containing data as should be saved in 2.0 teamwork table
 *
 * @param stdClass $old record from 1.9 teamwork table
 * @return stdClass
 */
function teamwork_upgrade_transform_instance(stdClass $old) {
    global $CFG;
    ...
    $new->phase         = teamwork::PHASE_CLOSED;
