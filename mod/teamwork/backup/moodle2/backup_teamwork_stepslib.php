...
/**
 * Defines all the backup steps that will be used by {@link backup_teamwork_activity_task}
 *
...
/**
 * Defines the complete teamwork structure for backup, with file and id annotations
 *
 * @link http://docs.moodle.org/dev/Teamwork for XML structure diagram
 */
class backup_teamwork_activity_structure_step extends backup_activity_structure_step {

    /**
     * Defines the structure of the 'teamwork' element inside the teamwork.xml file
     *
    ...
    protected function define_structure() {
        ...
        $teamwork = new backup_nested_element('teamwork', array('id'), array(
        ...
        $this->add_subplugin_structure('teamworkform', $teamwork, true);

        // grading evaluations data
        $this->add_subplugin_structure('teamworkeval', $teamwork, true);
        ...
        $this->add_subplugin_structure('teamworkform', $referenceassessment, true);
        ...
        $this->add_subplugin_structure('teamworkform', $exampleassessment, true);
        ...
        $this->add_subplugin_structure('teamworkform', $assessment, true);
        ...
        ////////////////////////////////////////////////////////////////////////
        $teamwork->add_child($examplesubmissions);
        ...
        $teamwork->set_source_table('teamwork', array('id' => backup::VAR_ACTIVITYID));

        $examplesubmission->set_source_sql("
            SELECT *
              FROM {teamwork_submissions}
             WHERE teamworkid = ? AND example = 1",
            array(backup::VAR_PARENTID));

        $referenceassessment->set_source_sql("
            SELECT *
              FROM {teamwork_assessments}
        ...
        if ($userinfo) {
            ...
            $submission->set_source_sql("
                SELECT *
                  FROM {teamwork_submissions}
                 WHERE teamworkid = ? AND example = 0",
                 array(backup::VAR_PARENTID));  // must use SQL here, for the same reason as above

            $assessment->set_source_table('teamwork_assessments', array('submissionid' => backup::VAR_PARENTID));

            $aggregation->set_source_table('teamwork_aggregations', array('teamworkid' => backup::VAR_PARENTID));
        }
        ...
        $teamwork->annotate_files('mod_teamwork', 'intro', null); // no itemid used
        $teamwork->annotate_files('mod_teamwork', 'instructauthors', null); // no itemid used
        $teamwork->annotate_files('mod_teamwork', 'instructreviewers', null); // no itemid used
        $teamwork->annotate_files('mod_teamwork', 'conclusion', null); // no itemid used

        $examplesubmission->annotate_files('mod_teamwork', 'submission_content', 'id');
        $examplesubmission->annotate_files('mod_teamwork', 'submission_attachment', 'id');

        $referenceassessment->annotate_files('mod_teamwork', 'overallfeedback_content', 'id');
        $referenceassessment->annotate_files('mod_teamwork', 'overallfeedback_attachment', 'id');

        $exampleassessment->annotate_files('mod_teamwork', 'overallfeedback_content', 'id');
        $exampleassessment->annotate_files('mod_teamwork', 'overallfeedback_attachment', 'id');

        $submission->annotate_files('mod_teamwork', 'submission_content', 'id');
        $submission->annotate_files('mod_teamwork', 'submission_attachment', 'id');

        $assessment->annotate_files('mod_teamwork', 'overallfeedback_content', 'id');
        $assessment->annotate_files('mod_teamwork', 'overallfeedback_attachment', 'id');

        // return the root element (teamwork), wrapped into standard activity structure
        return $this->prepare_activity_structure($teamwork);
    }
}
