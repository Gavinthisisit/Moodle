...
class mod_teamworkforum_external extends external_api {

    /**
     * Describes the parameters for get_teamworkforum.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.5
     */
    public static function get_teamworkforums_by_courses_parameters() {
    ...
    /**
     * Returns a list of teamworkforums in a provided list of courses,
     * if no list is provided all teamworkforums that the user can view
     * will be returned.
     *
     * @param array $courseids the course ids
     * @return array the teamworkforum details
     * @since Moodle 2.5
     */
    public static function get_teamworkforums_by_courses($courseids = array()) {
        global $CFG;

        require_once($CFG->dirroot . "/mod/teamworkforum/lib.php");

        $params = self::validate_parameters(self::get_teamworkforums_by_courses_parameters(), array('courseids' => $courseids));
        ...
        // Array to store the teamworkforums to return.
        $arrteamworkforums = array();

        // Ensure there are courseids to loop through.
        if (!empty($courseids)) {
            // Array of the courses we are going to retrieve the teamworkforums from.
            $dbcourses = array();
            // Mod info for courses.
            $modinfocourses = array();

            // Go through the courseids and return the teamworkforums.
            ...
            // Get the teamworkforums in this course. This function checks users visibility permissions.
            if ($teamworkforums = get_all_instances_in_courses("teamworkforum", $dbcourses)) {
                foreach ($teamworkforums as $teamworkforum) {

                    $course = $dbcourses[$teamworkforum->course];
                    $cm = $modinfocourses[$course->id]->get_cm($teamworkforum->coursemodule);
                    $context = context_module::instance($cm->id);

                    // Skip teamworkforums we are not allowed to see discussions.
                    if (!has_capability('mod/teamworkforum:viewdiscussion', $context)) {
                        continue;
                    }

                    // Format the intro before being returning using the format setting.
                    list($teamworkforum->intro, $teamworkforum->introformat) = external_format_text($teamworkforum->intro, $teamworkforum->introformat,
                                                                                    $context->id, 'mod_teamworkforum', 'intro', 0);
                    // Discussions count. This function does static request cache.
                    $teamworkforum->numdiscussions = teamworkforum_count_discussions($teamworkforum, $cm, $course);
                    $teamworkforum->cmid = $teamworkforum->coursemodule;

                    // Add the teamworkforum to the array to return.
                    $arrteamworkforums[$teamworkforum->id] = $teamworkforum;
                }
            }
        }

        return $arrteamworkforums;
    }

    /**
     * Describes the get_teamworkforum return value.
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
     public static function get_teamworkforums_by_courses_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'Forum id'),
                    'course' => new external_value(PARAM_TEXT, 'Course id'),
                    'type' => new external_value(PARAM_TEXT, 'The teamworkforum type'),
                    'name' => new external_value(PARAM_TEXT, 'Forum name'),
                    'intro' => new external_value(PARAM_RAW, 'The teamworkforum intro'),
                    ...
                    'numdiscussions' => new external_value(PARAM_INT, 'Number of discussions in the teamworkforum', VALUE_OPTIONAL)
                ), 'teamworkforum'
            )
        );
    }
    ...
    /**
     * Describes the parameters for get_teamworkforum_discussions.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.5
     * @deprecated Moodle 2.8 MDL-46458 - Please do not call this function any more.
     * @see get_teamworkforum_discussions_paginated
     */
    public static function get_teamworkforum_discussions_parameters() {
        return new external_function_parameters (
            array(
                'teamworkforumids' => new external_multiple_structure(new external_value(PARAM_INT, 'teamworkforum ID',
    ...
    /**
     * Returns a list of teamworkforum discussions as well as a summary of the discussion
     * in a provided list of teamworkforums.
     *
     * @param array $teamworkforumids the teamworkforum ids
     * @param int $limitfrom limit from SQL data
     * @param int $limitnum limit number SQL data
     *
     * @return array the teamworkforum discussion details
     * @since Moodle 2.5
     * @deprecated Moodle 2.8 MDL-46458 - Please do not call this function any more.
     * @see get_teamworkforum_discussions_paginated
     */
    public static function get_teamworkforum_discussions($teamworkforumids, $limitfrom = 0, $limitnum = 0) {
        global $CFG, $DB, $USER;

        require_once($CFG->dirroot . "/mod/teamworkforum/lib.php");

        // Validate the parameter.
        $params = self::validate_parameters(self::get_teamworkforum_discussions_parameters(),
            array(
                'teamworkforumids'  => $teamworkforumids,
                'limitfrom' => $limitfrom,
                'limitnum'  => $limitnum,
            ));
        $teamworkforumids  = $params['teamworkforumids'];
        $limitfrom = $params['limitfrom'];
        $limitnum  = $params['limitnum'];

        // Array to store the teamworkforum discussions to return.
        $arrdiscussions = array();
        // Keep track of the users we have looked up in the DB.
        $arrusers = array();

        // Loop through them.
        foreach ($teamworkforumids as $id) {
            // Get the teamworkforum object.
            $teamworkforum = $DB->get_record('teamworkforum', array('id' => $id), '*', MUST_EXIST);
            $course = get_course($teamworkforum->course);

            $modinfo = get_fast_modinfo($course);
            $teamworkforums  = $modinfo->get_instances_of('teamworkforum');
            $cm = $teamworkforums[$teamworkforum->id];
            ...
            require_capability('mod/teamworkforum:viewdiscussion', $modcontext);

            // Get the discussions for this teamworkforum.
            $params = array();
            ...
            $select = "teamworkforum = ? $groupselect";

            if ($discussions = $DB->get_records_select('teamworkforum_discussions', $select, $params, 'timemodified DESC', '*',
                                                            $limitfrom, $limitnum)) {

                // Check if they can view full names.
                $canviewfullname = has_capability('moodle/site:viewfullnames', $modcontext);
                // Get the unreads array, this takes a teamworkforum id and returns data for all discussions.
                $unreads = array();
                if ($cantrack = teamworkforum_tp_can_track_teamworkforums($teamworkforum)) {
                    if ($teamworkforumtracked = teamworkforum_tp_is_tracked($teamworkforum)) {
                        $unreads = teamworkforum_get_discussions_unread($cm);
                    }
                }
                // The teamworkforum function returns the replies for all the discussions in a given teamworkforum.
                $replies = teamworkforum_count_discussion_replies($id);

                foreach ($discussions as $discussion) {
                    // This function checks capabilities, timed discussions, groups and qanda teamworkforums posting.
                    if (!teamworkforum_user_can_see_discussion($teamworkforum, $discussion, $modcontext)) {
                        continue;
                    }
                    ...
                    // Get the subject.
                    $subject = $DB->get_field('teamworkforum_posts', 'subject', array('id' => $discussion->firstpost), MUST_EXIST);
                    // Create object to return.
                    $return = new stdClass();
                    $return->id = (int) $discussion->id;
                    $return->course = $discussion->course;
                    $return->teamworkforum = $discussion->teamworkforum;
                    ...
                    $return->numunread = '';
                    if ($cantrack && $teamworkforumtracked) {
                    ...
                    $lastpost = $DB->get_record('teamworkforum_posts', array('id' => $return->lastpost), '*', MUST_EXIST);
    ...
    /**
     * Describes the get_teamworkforum_discussions return value.
     *
     * @return external_single_structure
     * @since Moodle 2.5
     * @deprecated Moodle 2.8 MDL-46458 - Please do not call this function any more.
     * @see get_teamworkforum_discussions_paginated
     */
     public static function get_teamworkforum_discussions_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'Forum id'),
                    'course' => new external_value(PARAM_INT, 'Course id'),
                    'teamworkforum' => new external_value(PARAM_INT, 'The teamworkforum id'),
                    ...
                    'numunread' => new external_value(PARAM_TEXT, 'The number of unread posts, blank if this value is
                        not available due to teamworkforum settings.'),
    ...
    /**
     * Describes the parameters for get_teamworkforum_discussion_posts.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.7
     */
    public static function get_teamworkforum_discussion_posts_parameters() {
        return new external_function_parameters (
            array(
    ...
    /**
     * Returns a list of teamworkforum posts for a discussion
     *
     * @param int $discussionid the post ids
     * @param string $sortby sort by this element (id, created or modified)
     * @param string $sortdirection sort direction: ASC or DESC
     *
     * @return array the teamworkforum post details
     * @since Moodle 2.7
     */
    public static function get_teamworkforum_discussion_posts($discussionid, $sortby = "created", $sortdirection = "DESC") {
        global $CFG, $DB, $USER;
        ...
        // Validate the parameter.
        $params = self::validate_parameters(self::get_teamworkforum_discussion_posts_parameters(),
            array(
        ...
        // This require must be here, see mod/teamworkforum/discuss.php.
        require_once($CFG->dirroot . "/mod/teamworkforum/lib.php");

        // Check they have the view teamworkforum capability.
        ...
        if (! $post = teamworkforum_get_post_full($discussion->firstpost)) {
            throw new moodle_exception('notexists', 'teamworkforum');
        }
        ...
        if (!teamworkforum_user_can_see_post($teamworkforum, $discussion, $post, null, $cm)) {
            throw new moodle_exception('noviewdiscussionspermission', 'teamworkforum');
        }
        ...
        $allposts = teamworkforum_get_all_discussion_posts($discussion->id, $sort, $teamworkforumtracked);
        ...
        foreach ($allposts as $post) {
            ...
            // Function teamworkforum_get_all_discussion_posts adds postread field.
            ...
            list($post->message, $post->messageformat) =
                external_format_text($post->message, $post->messageformat, $modcontext->id, 'mod_teamworkforum', 'post', $post->id);
            ...
            if (!empty($post->attachment)) {
                ...
                $fs = get_file_storage();
                if ($files = $fs->get_area_files($modcontext->id, 'mod_teamworkforum', 'attachment', $post->id, "filename", false)) {
                    foreach ($files as $file) {
                        $filename = $file->get_filename();
                        $fileurl = moodle_url::make_webservice_pluginfile_url(
                                        $modcontext->id, 'mod_teamworkforum', 'attachment', $post->id, '/', $filename);
    ...
    /**
     * Describes the get_teamworkforum_discussion_posts return value.
     *
     * @return external_single_structure
     * @since Moodle 2.7
     */
    public static function get_teamworkforum_discussion_posts_returns() {
    ...
    /**
     * Describes the parameters for get_teamworkforum_discussions_paginated.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.8
     */
    public static function get_teamworkforum_discussions_paginated_parameters() {
        return new external_function_parameters (
            array(
                'teamworkforumid' => new external_value(PARAM_INT, 'teamworkforum instance id', VALUE_REQUIRED),
                'sortby' => new external_value(PARAM_ALPHA,
    ...
    /**
     * Returns a list of teamworkforum discussions optionally sorted and paginated.
     *
     * @param int $teamworkforumid the teamworkforum instance id
     * @param string $sortby sort by this element (id, timemodified, timestart or timeend)
     * @param string $sortdirection sort direction: ASC or DESC
     * @param int $page page number
     * @param int $perpage items per page
     *
     * @return array the teamworkforum discussion details including warnings
     * @since Moodle 2.8
     */
    public static function get_teamworkforum_discussions_paginated($teamworkforumid, $sortby = 'timemodified', $sortdirection = 'DESC',
                                                    $page = -1, $perpage = 0) {
        global $CFG, $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_teamworkforum_discussions_paginated_parameters(),
            array(
                'teamworkforumid' => $teamworkforumid,
                'sortby' => $sortby,
                'sortdirection' => $sortdirection,
                'page' => $page,
                'perpage' => $perpage
        ...
        // Compact/extract functions are not recommended.
        $teamworkforumid        = $params['teamworkforumid'];
        $sortby         = $params['sortby'];
        $sortdirection  = $params['sortdirection'];
        $page           = $params['page'];
        ...
        $alldiscussions = teamworkforum_get_discussions($cm, $sort, true, -1, -1, true, $page, $perpage);
        ...
        if ($alldiscussions) {
            ...
            // The teamworkforum function returns the replies for all the discussions in a given teamworkforum.
            $replies = teamworkforum_count_discussion_replies($teamworkforumid, $sort, -1, $page, $perpage);

            foreach ($alldiscussions as $discussion) {

                // This function checks for qanda teamworkforums.
                // Note that the teamworkforum_get_discussions returns as id the post id, not the discussion id so we need to do this.
                $discussionrec = clone $discussion;
                $discussionrec->id = $discussion->discussion;
                if (!teamworkforum_user_can_see_discussion($teamworkforum, $discussionrec, $modcontext)) {
                    $warning = array();
                    // Function teamworkforum_get_discussions returns teamworkforum_posts ids not teamworkforum_discussions ones.
                    $warning['item'] = 'post';
                ...
                list($discussion->message, $discussion->messageformat) =
                    external_format_text($discussion->message, $discussion->messageformat,
                                            $modcontext->id, 'mod_teamworkforum', 'post', $discussion->id);
                ...
                if (!empty($discussion->attachment)) {
                    ...
                    $fs = get_file_storage();
                    if ($files = $fs->get_area_files($modcontext->id, 'mod_teamworkforum', 'attachment',
                        ...
                        foreach ($files as $file) {
                            ...
                            $discussion->attachments[] = array(
                                ...
                                'fileurl'  => file_encode_url($CFG->wwwroot.'/webservice/pluginfile.php',
                                                '/'.$modcontext->id.'/mod_teamworkforum/attachment/'.$discussion->id.'/'.$filename)
                            );
                        }
                    }
                }
    ...
    /**
     * Describes the get_teamworkforum_discussions_paginated return value.
     *
     * @return external_single_structure
     * @since Moodle 2.8
     */
    public static function get_teamworkforum_discussions_paginated_returns() {
        return new external_single_structure(
            array(
     ...
     * @since Moodle 2.9
     */
    public static function view_teamworkforum_parameters() {
        return new external_function_parameters(
            array(
                'teamworkforumid' => new external_value(PARAM_INT, 'teamworkforum instance id')
            )
        );
    }

    /**
     * Simulate the teamworkforum/view.php web interface page: trigger events, completion, etc...
     *
     * @param int $teamworkforumid the teamworkforum instance id
     ...
     */
    public static function view_teamworkforum($teamworkforumid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/teamworkforum/lib.php");

        $params = self::validate_parameters(self::view_teamworkforum_parameters(),
                                            array(
                                                'teamworkforumid' => $teamworkforumid
                                            ));
        $warnings = array();
        ...
        $teamworkforum = $DB->get_record('teamworkforum', array('id' => $params['teamworkforumid']), 'id', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($teamworkforum, 'teamworkforum');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        require_capability('mod/teamworkforum:viewdiscussion', $context, null, true, 'noviewdiscussionspermission', 'teamworkforum');

        // Call the teamworkforum/lib API.
     ...
     * @since Moodle 2.9
     */
    public static function view_teamworkforum_returns() {
        return new external_single_structure(
            array(
     ...
     * @since Moodle 2.9
     */
    public static function view_teamworkforum_discussion_parameters() {
        return new external_function_parameters(
            array(
    ...
    /**
     * Simulate the teamworkforum/discuss.php web interface page: trigger events
     *
     ...
     * @throws moodle_exception
     */
    public static function view_teamworkforum_discussion($discussionid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/teamworkforum/lib.php");

        $params = self::validate_parameters(self::view_teamworkforum_discussion_parameters(),
                                            array(
                                                'discussionid' => $discussionid
                                            ));
        $warnings = array();

        $discussion = $DB->get_record('teamworkforum_discussions', array('id' => $params['discussionid']), '*', MUST_EXIST);
        $teamworkforum = $DB->get_record('teamworkforum', array('id' => $discussion->teamworkforum), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($teamworkforum, 'teamworkforum');
     ...
     * @since Moodle 2.9
     */
    public static function view_teamworkforum_discussion_returns() {
        return new external_single_structure(
            array(
                'status' => new external_value(PARAM_BOOL, 'status: true if success'),
                'warnings' => new external_warnings()
