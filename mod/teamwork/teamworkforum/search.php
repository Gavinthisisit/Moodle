...
$teamworkforumid = trim(optional_param('teamworkforumid', 0, PARAM_INT));      // ForumID to search for
...
if (empty($search)) {   // Check the other parameters instead
    ...
    if (!empty($teamworkforumid)) {
        $search .= ' teamworkforumid:'.$teamworkforumid;
    }
    if (!empty($user)) {
        $search .= ' '.teamworkforum_clean_search_terms($user, 'user:');
    }
    if (!empty($subject)) {
        $search .= ' '.teamworkforum_clean_search_terms($subject, 'subject:');
    }
    if (!empty($fullwords)) {
        $search .= ' '.teamworkforum_clean_search_terms($fullwords, '+');
    }
    if (!empty($notwords)) {
        $search .= ' '.teamworkforum_clean_search_terms($notwords, '-');
    }
    if (!empty($phrase)) {
        $search .= ' "'.$phrase.'"';
    }
...
if ($search) {
    $search = teamworkforum_clean_search_terms($search);
}
...
$strsearch = get_string("search", "teamworkforum");
$strsearchresults = get_string("searchresults", "teamworkforum");
$strpage = get_string("page");

if (!$search || $showform) {

    $PAGE->navbar->add($strteamworkforums, new moodle_url('/mod/teamworkforum/index.php', array('id'=>$course->id)));
    $PAGE->navbar->add(get_string('advancedsearch', 'teamworkforum'));

    $PAGE->set_title($strsearch);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();

    teamworkforum_print_big_search_form($course);
    echo $OUTPUT->footer();
    exit;
}
...
$searchterms = str_replace('teamworkforumid:', 'instance:', $search);
$searchterms = explode(' ', $searchterms);

$searchform = teamworkforum_search_form($course, $search);

$PAGE->navbar->add($strsearch, new moodle_url('/mod/teamworkforum/search.php', array('id'=>$course->id)));
$PAGE->navbar->add($strsearchresults);
if (!$posts = teamworkforum_search_posts($searchterms, $course->id, $page*$perpage, $perpage, $totalcount)) {
    $PAGE->set_title($strsearchresults);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading($strteamworkforums, 2);
    echo $OUTPUT->heading($strsearchresults, 3);
    echo $OUTPUT->heading(get_string("noposts", "teamworkforum"), 4);
...
echo '<a href="search.php?id='.$course->id.
                         ...
                         '&amp;datefrom='.$datefrom.
                         '&amp;showform=1'.
                         '">'.get_string('advancedsearch','teamworkforum').'...</a>';
echo '</div>';
...
foreach ($posts as $post) {
    ...
    // Replace the simple subject with the three items teamworkforum name -> thread name -> subject
    ...
    $fullsubject = "<a href=\"view.php?f=$teamworkforum->id\">".format_string($teamworkforum->name,true)."</a>";
    ...
    //Unfortunately seem to have do this individually as posts may be from different teamworkforums
    ...
    // the start of the message text. However, do not do the highlighting here. teamworkforum_print_post
    ...
    if ($missing_terms) {
        $strmissingsearchterms = get_string('missingsearchterms','teamworkforum');
        $post->message = '<p class="highlight2">'.$strmissingsearchterms.' '.$missing_terms.'</p>'.$post->message;
    }

    // Prepare a link to the post in context, to be displayed after the teamworkforum post.
    $fulllink = "<a href=\"discuss.php?d=$post->discussion#p$post->id\">".get_string("postincontext", "teamworkforum")."</a>";
...
echo $OUTPUT->footer();
  ...
  */
function teamworkforum_print_big_search_form($course) {
    global $CFG, $DB, $words, $subject, $phrase, $user, $userid, $fullwords, $notwords, $datefrom, $dateto, $PAGE, $OUTPUT;

    echo $OUTPUT->box(get_string('searchteamworkforumintro', 'teamworkforum'), 'searchbox boxaligncenter', 'intro');

    echo $OUTPUT->box_start('generalbox boxaligncenter');

    echo html_writer::script('', $CFG->wwwroot.'/mod/teamworkforum/teamworkforum.js');

    echo '<form id="searchform" action="search.php" method="get">';
    echo '<table cellpadding="10" class="searchbox" id="form">';

    echo '<tr>';
    echo '<td class="c0"><label for="words">'.get_string('searchwords', 'teamworkforum').'</label>';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="phrase">'.get_string('searchphrase', 'teamworkforum').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="phrase" id="phrase" value="'.s($phrase, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="notwords">'.get_string('searchnotwords', 'teamworkforum').'</label></td>';
    ...
    if ($DB->get_dbfamily() == 'mysql' || $DB->get_dbfamily() == 'postgres') {
        echo '<tr>';
        echo '<td class="c0"><label for="fullwords">'.get_string('searchfullwords', 'teamworkforum').'</label></td>';
        echo '<td class="c1"><input type="text" size="35" name="fullwords" id="fullwords" value="'.s($fullwords, true).'" alt="" /></td>';
        echo '</tr>';
    }

    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdatefrom', 'teamworkforum').'</td>';
    echo '<td class="c1">';
    if (empty($datefrom)) {
        $datefromchecked = '';
    ...
    echo '<input name="timefromrestrict" type="checkbox" value="1" alt="'.get_string('searchdatefrom', 'teamworkforum').'" onclick="return lockoptions(\'searchform\', \'timefromrestrict\', timefromitems)" '.  $datefromchecked . ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0">'.get_string('searchdateto', 'teamworkforum').'</td>';
    echo '<td class="c1">';
    if (empty($dateto)) {
        $datetochecked = '';
        $dateto = time()+3600;
    ...
    echo '<input name="timetorestrict" type="checkbox" value="1" alt="'.get_string('searchdateto', 'teamworkforum').'" onclick="return lockoptions(\'searchform\', \'timetorestrict\', timetoitems)" ' .$datetochecked. ' /> ';
    ...
    echo '<tr>';
    echo '<td class="c0"><label for="menuteamworkforumid">'.get_string('searchwhichteamworkforums', 'teamworkforum').'</label></td>';
    echo '<td class="c1">';
    echo html_writer::select(teamworkforum_menu_list($course), 'teamworkforumid', '', array(''=>get_string('allteamworkforums', 'teamworkforum')));
    echo '</td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="subject">'.get_string('searchsubject', 'teamworkforum').'</label></td>';
    echo '<td class="c1"><input type="text" size="35" name="subject" id="subject" value="'.s($subject, true).'" alt="" /></td>';
    echo '</tr>';

    echo '<tr>';
    echo '<td class="c0"><label for="user">'.get_string('searchuser', 'teamworkforum').'</label></td>';
    ...
    echo '<input type="submit" value="'.get_string('searchteamworkforums', 'teamworkforum').'" alt="" /></td>';
    echo '</tr>';
 ...
 */
function teamworkforum_clean_search_terms($words, $prefix='') {
 ...
 /**
  * Retrieve a list of the teamworkforums that this user can view.
  *
  * @param stdClass $course The Course to use.
  * @return array A set of formatted teamworkforum names stored against the teamworkforum id.
  */
function teamworkforum_menu_list($course)  {
    $menu = array();
    ...
    foreach ($modinfo->instances['teamworkforum'] as $cm) {
        if (!$cm->uservisible) {
            continue;
        }
