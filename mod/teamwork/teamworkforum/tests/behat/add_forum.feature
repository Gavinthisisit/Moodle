@mod @mod_teamworkforum @_file_upload
Feature: Add teamworkforum activities and discussions
  In order to discuss topics with other users
  As a teacher
  I need to add teamworkforum activities to moodle courses

  @javascript
  Scenario: Add a teamworkforum and a discussion attaching files
    ...
    And I follow "Course 1"
    When I add a new discussion to "Test teamworkforum name" teamworkforum with:
      | Subject | Post with attachment |
      | Message | This is the body |
      | Attachment | lib/tests/fixtures/empty.txt |
    And I reply "Forum post 1" post from "Test teamworkforum name" teamworkforum with:
