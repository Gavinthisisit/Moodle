...
/**
 * The teamworkforum module mail generation tests.
 *
...
class mod_teamworkforum_mail_testcase extends advanced_testcase {
    ...
    /**
     * Perform message inbound setup for the mod_teamworkforum reply handler.
     */
    protected function helper_spoof_message_inbound_setup() {
        ...
        $record = $DB->get_record('messageinbound_handlers', array('classname' => '\mod_teamworkforum\message\inbound\reply_handler'));
        $record->enabled = true;
     ...
     */
    protected function helper_post_to_teamworkforum($teamworkforum, $author, $fields = array()) {
        global $DB;
    ...
    protected function helper_update_post_time($post, $factor) {
        ...
        $DB->set_field('teamworkforum_posts', 'created', $post->created + $factor, array('id' => $post->id));
    }
    ...
    protected function helper_update_subscription_time($user, $discussion, $factor) {
        ...
        $sub = $DB->get_record('teamworkforum_discussion_subs', array('userid' => $user->id, 'discussion' => $discussion->id));

        // Update the subscription to have a preference in the past.
        $DB->set_field('teamworkforum_discussion_subs', 'preference', $sub->preference + $factor, array('id' => $sub->id));
    }
     ...
     * @param stdClass $author The author to post as
     * @return stdClass The teamworkforum post
     */
    protected function helper_post_to_discussion($teamworkforum, $discussion, $author) {
        global $DB;
    ...
    /**
     * Run the teamworkforum cron, and check that the specified post was sent the
     * specified number of times.
     *
     * @param stdClass $post The teamworkforum post object
    ...
    /**
     * Run the teamworkforum cron, and check that the specified posts were sent the
     * specified number of times.
     *
     * @param stdClass $post The teamworkforum post object
     * @param integer $expected The number of times that the post should have been sent
    ...
    public function test_subscription_disabled() {
        ...
        \mod_teamworkforum\subscriptions::subscribe_user($recipient->id, $teamworkforum);

        $this->assertEquals($expected, $DB->count_records('teamworkforum_subscriptions', array(
            'userid'        => $recipient->id,
            'teamworkforum'         => $teamworkforum->id,
        )));

        // Run cron and check that the expected number of users received the notification.
        list($discussion, $post) = $this->helper_post_to_teamworkforum($teamworkforum, $recipient);
        $messages = $this->helper_run_cron_check_count($post, $expected);

        // Unsubscribe the user again.
        \mod_teamworkforum\subscriptions::unsubscribe_user($recipient->id, $teamworkforum);
        ...
        \mod_teamworkforum\subscriptions::subscribe_user_to_discussion($recipient->id, $discussion);
        $this->helper_update_subscription_time($recipient, $discussion, -60);

        $reply = $this->helper_post_to_discussion($teamworkforum, $discussion, $author);
    ...
    public function test_automatic_with_unsubscribed_user() {
        ...
        // Unsubscribe the 'author' user from the teamworkforum.
    ...
    public function test_optional_with_subscribed_user() {
        ...
        // Subscribe the 'recipient' user from the teamworkforum.
        \mod_teamworkforum\subscriptions::subscribe_user($recipient->id, $teamworkforum);
    ...
    public function test_automatic_with_unsubscribed_discussion() {
        ...
        $this->assertTrue(\mod_teamworkforum\subscriptions::is_subscribed($recipient->id, $teamworkforum, $discussion->id));
    ...
    public function test_automatic_with_subscribed_discussion_in_unsubscribed_teamworkforum() {
        $this->resetAfterTest(true);
        ...
        // We expect just the user subscribed to the teamworkforum to receive this post at the moment as the discussion
    ...
    public function test_optional_with_unsubscribed_discussion_in_subscribed_teamworkforum() {
        $this->resetAfterTest(true);
        ...
        \mod_teamworkforum\subscriptions::unsubscribe_user_from_discussion($recipient->id, $discussion);
    ...
    /**
     * Test that a user unsubscribed from a teamworkforum who has subscribed to a discussion, only receives posts made after
     * they subscribed to the discussion.
     */
    public function test_teamworkforum_discussion_subscription_teamworkforum_unsubscribed_discussion_subscribed_after_post() {
        $this->resetAfterTest(true);
    ...
    public function test_teamworkforum_message_inbound_multiple_posts() {
        $this->resetAfterTest(true);
        ...
        // Ensure that messageinbound is enabled and configured for the teamworkforum handler.
        $this->helper_spoof_message_inbound_setup();

        $author->emailstop = '0';
        set_user_preference('message_provider_mod_teamworkforum_posts_loggedoff', 'email', $author);
        set_user_preference('message_provider_mod_teamworkforum_posts_loggedin', 'email', $author);
    ...
    public function test_long_subject() {
        ...
        // Post a discussion to the teamworkforum.
        $subject = 'This is the very long teamworkforum post subject that somebody was very kind of leaving, it is intended to check if long subject comes in mail correctly. Thank you.';
        $a = (object)array('courseshortname' => $course->shortname, 'teamworkforumname' => $teamworkforum->name, 'subject' => $subject);
        $expectedsubject = get_string('postmailsubject', 'teamworkforum', $a);
        list($discussion, $post) = $this->helper_post_to_teamworkforum($teamworkforum, $author, array('name' => $subject));
