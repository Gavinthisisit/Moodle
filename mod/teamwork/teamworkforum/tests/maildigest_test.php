...
class mod_teamworkforum_maildigest_testcase extends advanced_testcase {
    ...
    /**
     * Setup a user, course, and teamworkforums.
     *
     * @return stdClass containing the list of teamworkforums, courses, teamworkforumids,
    ...
    protected function helper_setup_user_in_course() {
        ...
        $return->teamworkforums = new stdClass();
        $return->teamworkforumids = array();
        ...
        // Create teamworkforums.
        $record = new stdClass();
        $record->course = $return->courses->course1->id;
        $record->forcesubscribe = 1;

        $return->teamworkforums->teamworkforum1 = $this->getDataGenerator()->create_module('teamworkforum', $record);
        $return->teamworkforumsids[] = $return->teamworkforums->teamworkforum1->id;

        $return->teamworkforums->teamworkforum2 = $this->getDataGenerator()->create_module('teamworkforum', $record);
        $return->teamworkforumsids[] = $return->teamworkforums->teamworkforum2->id;

        // Check the teamworkforum was correctly created.
        list ($test, $params) = $DB->get_in_or_equal($return->teamworkforumsids);
    ...
    /**
     * Helper to falsify all teamworkforum post records for a digest run.
     */
    protected function helper_force_digest_mail_times() {
        ...
        $DB->set_field('teamworkforum_posts', 'modified', $digesttime, array('mailed' => 0));
        $DB->set_field('teamworkforum_posts', 'created', $digesttime, array('mailed' => 0));
    }
    ...
    public function test_set_maildigest() {
        ...
        $teamworkforum1 = $helper->teamworkforums->teamworkforum1;

        // Set to the user.
        self::setUser($helper->user);

        // Confirm that there is no current value.
        $currentsetting = $DB->get_record('teamworkforum_digests', array(
            'teamworkforum' => $teamworkforum1->id,
            'userid' => $user->id,
        ));
        ...
        teamworkforum_set_user_maildigest($teamworkforum1, 0, $user);
        $currentsetting = $DB->get_record('teamworkforum_digests', array(
            'teamworkforum' => $teamworkforum1->id,
            'userid' => $user->id,
        ));
        ...
        teamworkforum_set_user_maildigest($teamworkforum1, 1, $user);
        $currentsetting = $DB->get_record('teamworkforum_digests', array(
            'teamworkforum' => $teamworkforum1->id,
            'userid' => $user->id,
        ));
        ...
        teamworkforum_set_user_maildigest($teamworkforum1, 2, $user);
        $currentsetting = $DB->get_record('teamworkforum_digests', array(
            'teamworkforum' => $teamworkforum1->id,
            'userid' => $user->id,
        ));
        ...
        teamworkforum_set_user_maildigest($teamworkforum1, 42, $user);
    }
    ...
    public function test_get_user_digest_options_default() {
        ...
        $digestoptions = array(
            '0' => get_string('emaildigestoffshort', 'mod_teamworkforum'),
            '1' => get_string('emaildigestcompleteshort', 'mod_teamworkforum'),
            '2' => get_string('emaildigestsubjectsshort', 'mod_teamworkforum'),
        );

        // The default settings is 0.
        $this->assertEquals(0, $user->maildigest);
        $options = teamworkforum_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_teamworkforum', $digestoptions[0]));

        // Update the setting to 1.
        $USER->maildigest = 1;
        $this->assertEquals(1, $USER->maildigest);
        $options = teamworkforum_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_teamworkforum', $digestoptions[1]));

        // Update the setting to 2.
        $USER->maildigest = 2;
        $this->assertEquals(2, $USER->maildigest);
        $options = teamworkforum_get_user_digest_options();
        $this->assertEquals($options[-1], get_string('emaildigestdefault', 'mod_teamworkforum', $digestoptions[2]));
    }
    ...
    public function test_cron_no_posts() {
        ...
        // Initially the teamworkforum cron should generate no messages as we've made no posts.
    ...
    public function test_cron_profile_single_mails() {
        ...
        $teamworkforum1 = $userhelper->teamworkforums->teamworkforum1;
        $teamworkforum2 = $userhelper->teamworkforums->teamworkforum2;

        // Add some discussions to the teamworkforums.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->mailnow = 1;

        // Add 5 discussions to teamworkforum 1.
        ...
        // Add 5 discussions to teamworkforum 2.
        ...
        // Set the maildigest preference for teamworkforum1 to default.
        teamworkforum_set_user_maildigest($teamworkforum1, -1, $user);

        // Set the maildigest preference for teamworkforum2 to default.
        teamworkforum_set_user_maildigest($teamworkforum2, -1, $user);

        // No digests mails should be sent, but 10 teamworkforum mails will be sent.
    ...
    public function test_cron_profile_digest_email() {
        ...
        // Add a discussion to the teamworkforums.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->userid = $user->id;
        $record->mailnow = 1;
    ...
    /**
     * Sends several notifications to one user as:
     * * daily digests coming from the per-teamworkforum setting; and
    ...
    public function test_cron_mixed_email_1() {
        ...
        // Set the maildigest preference for teamworkforum1 to digest.
        teamworkforum_set_user_maildigest($teamworkforum1, 1, $user);

        // Set the maildigest preference for teamworkforum2 to default (single).
        teamworkforum_set_user_maildigest($teamworkforum2, -1, $user);
    ...
    /**
     * Sends several notifications to one user as:
     * * single e-mails from the per-teamworkforum setting; and
    ...
    public function test_cron_mixed_email_2() {
        ...
        // Set the maildigest preference for teamworkforum2 to single.
        teamworkforum_set_user_maildigest($teamworkforum2, 0, $user);
    ...
    /**
     * Sends several notifications to one user as:
     * * daily digests coming from the per-teamworkforum setting.
     */
    public function test_cron_teamworkforum_digest_email() {
        global $DB, $CFG;
        ...
        // Set the maildigest preference for teamworkforum1 to digest (complete).
        teamworkforum_set_user_maildigest($teamworkforum1, 1, $user);

        // Set the maildigest preference for teamworkforum2 to digest (short).
        teamworkforum_set_user_maildigest($teamworkforum2, 2, $user);
