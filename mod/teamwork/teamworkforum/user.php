...
$url = new moodle_url('/mod/teamworkforum/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
if ($isspecificcourse) {
    ...
    $courses = teamworkforum_get_courses_user_posted_in($user, $discussionsonly);
}
...
$result = teamworkforum_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_teamworkforum');

    // Get the page heading
    if ($isspecificcourse) {
        $pageheading = format_string($course->fullname, true, array('context' => $coursecontext));
    } else {
        $pageheading = get_string('pluginname', 'mod_teamworkforum');
    }
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'teamworkforum');
        } else {
            $notification = get_string('nopostsmadebyyou', 'teamworkforum');
        }
        // These are the user's teamworkforum interactions.
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'teamworkforum'), new moodle_url('/mod/teamworkforum/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'teamworkforum'), new moodle_url('/mod/teamworkforum/user.php',
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'teamworkforum', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'teamworkforum', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'teamworkforum');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('teamworkforum_discussions', 'id', array_unique($discussions));

//todo Rather than retrieving the ratings for each post individually it would be nice to do them in groups
//however this requires creating arrays of posts with each array containing all of the posts from a particular teamworkforum,
...
foreach ($result->posts as $post) {
    if (!isset($result->teamworkforums[$post->teamworkforum]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $teamworkforum = $result->teamworkforums[$post->teamworkforum];
    $cm = $teamworkforum->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $teamworkforumurl = new moodle_url('/mod/teamworkforum/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/teamworkforum/discuss.php', array('d' => $post->discussion));
    ...
    if ($teamworkforum->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($teamworkforum->type == 'single' or !$post->discussion) {
            $ratingoptions->returnurl = $teamworkforumurl;
        } else {
    ...
    $teamworkforumname = format_string($teamworkforum->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($teamworkforumurl, $teamworkforumname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $teamworkforumname);
    }
    if ($teamworkforum->type != 'single') {
        ...
        if ($post->parent != 0) {
            ...
            if (!$isspecificcourse && !$hasparentaccess) {
                $fullsubjects[] .= html_writer::link(new moodle_url('/mod/teamworkforum/discuss.php', array('d' => $post->discussion, 'parent' => $post->id)), $postname);
            } else {
    ...
    $fulllink = html_writer::link($discussionurl, get_string("postincontext", "teamworkforum"));

    $postoutput[] = teamworkforum_print_post($post, $discussion, $teamworkforum, $cm, $course, false, false, false, $fulllink, '', null, true, null, true);
}

$userfullname = fullname($user);

if ($discussionsonly) {
    $inpageheading = get_string('discussionsstartedby', 'mod_teamworkforum', $userfullname);
} else {
    $inpageheading = get_string('postsmadebyuser', 'mod_teamworkforum', $userfullname);
}
if ($isspecificcourse) {
    $a = new stdClass;
    ...
    if ($discussionsonly) {
        $pagetitle = get_string('discussionsstartedbyuserincourse', 'mod_teamworkforum', $a);
    } else {
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_teamworkforum', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'teamworkforum', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'teamworkforum'));
}
