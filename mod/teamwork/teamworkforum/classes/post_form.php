...
/**
 * File containing the form definition to post in the teamworkforum.
 *
...
/**
 * Class to post in a teamworkforum.
 *
 * @package   mod_teamworkforum
 * @copyright Jamie Pratt <me@jamiep.org>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_teamworkforum_post_form extends moodleform {

    /**
     * Returns the options array to use in filemanager for teamworkforum attachments
     *
     * @param stdClass $teamworkforum
     * @return array
     */
    public static function attachment_options($teamworkforum) {
        global $COURSE, $PAGE, $CFG;
        $maxbytes = get_user_max_upload_file_size($PAGE->context, $CFG->maxbytes, $COURSE->maxbytes, $teamworkforum->maxbytes);
        return array(
            'subdirs' => 0,
            'maxbytes' => $maxbytes,
            'maxfiles' => $teamworkforum->maxattachments,
            'accepted_types' => '*',
    ...
    /**
     * Returns the options array to use in teamworkforum text editor
     *
    ...
    public static function editor_options(context_module $context, $postid) {
        ...
        return array(
            ...
            'subdirs' => file_area_contains_subdirs($context, 'mod_teamworkforum', 'post', $postid)
        );
    }
    ...
    function definition() {
        ...
        $teamworkforum = $this->_customdata['teamworkforum'];
        ...
        $mform->addElement('text', 'subject', get_string('subject', 'teamworkforum'), 'size="48"');
        ...
        if (\mod_teamworkforum\subscriptions::is_forcesubscribed($teamworkforum)) {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'teamworkforum'));
            $mform->freeze('discussionsubscribe');
            $mform->setDefaults('discussionsubscribe', 0);
            $mform->addHelpButton('discussionsubscribe', 'forcesubscribed', 'teamworkforum');

        } else if (\mod_teamworkforum\subscriptions::subscription_disabled($teamworkforum) && !$manageactivities) {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'teamworkforum'));
            $mform->freeze('discussionsubscribe');
            $mform->setDefaults('discussionsubscribe', 0);
            $mform->addHelpButton('discussionsubscribe', 'disallowsubscription', 'teamworkforum');

        } else {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'teamworkforum'));
            $mform->addHelpButton('discussionsubscribe', 'discussionsubscription', 'teamworkforum');
        }

        if (!empty($teamworkforum->maxattachments) && $teamworkforum->maxbytes != 1 && has_capability('mod/teamworkforum:createattachment', $modcontext))  {  //  1 = No attachments at all
            $mform->addElement('filemanager', 'attachments', get_string('attachment', 'teamworkforum'), null, self::attachment_options($teamworkforum));
            $mform->addHelpButton('attachments', 'attachment', 'teamworkforum');
        }

        if (empty($post->id) && $manageactivities) {
            $mform->addElement('checkbox', 'mailnow', get_string('mailnow', 'teamworkforum'));
        }

        if (!empty($CFG->teamworkforum_enabletimedposts) && !$post->parent && has_capability('mod/teamworkforum:viewhiddentimedposts', $coursecontext)) { // hack alert
            $mform->addElement('header', 'displayperiod', get_string('displayperiod', 'teamworkforum'));

            $mform->addElement('date_selector', 'timestart', get_string('displaystart', 'teamworkforum'), array('optional'=>true));
            $mform->addHelpButton('timestart', 'displaystart', 'teamworkforum');

            $mform->addElement('date_selector', 'timeend', get_string('displayend', 'teamworkforum'), array('optional'=>true));
            $mform->addHelpButton('timeend', 'displayend', 'teamworkforum');
        ...
        if ($groupmode = groups_get_activity_groupmode($cm, $course)) {
            ...
            foreach ($groupdata as $groupid => $group) {
                ...
                if (teamworkforum_user_can_post_discussion($teamworkforum, $groupid, null, $cm, $modcontext)) {
            ...
            $canposttoowngroups = $canposttoowngroups && has_capability('mod/teamworkforum:canposttomygroups', $modcontext);
            if ($canposttoowngroups) {
                ...
                // user is a member of.
                $mform->addElement('checkbox', 'posttomygroups', get_string('posttomygroups', 'teamworkforum'));
                $mform->addHelpButton('posttomygroups', 'posttomygroups', 'teamworkforum');
            ...
            if (teamworkforum_user_can_post_discussion($teamworkforum, -1, null, $cm, $modcontext)) {
            ...
            // We allow this because the post may have been moved from another teamworkforum where groups are not available.
            // We show this even if no groups are available as groups *may* have been available but now are not.
            $canselectgroupformove = $groupcount && !empty($post->edit) && has_capability('mod/teamworkforum:movediscussions', $modcontext);
        ...
        if (isset($post->edit)) { // hack alert
            ...
            $submit_string = get_string('posttoteamworkforum', 'teamworkforum');
        }
        ...
        $mform->addElement('hidden', 'teamworkforum');
        $mform->setType('teamworkforum', PARAM_INT);
    ...
    function validation($data, $files) {
        ...
        if (($data['timeend']!=0) && ($data['timestart']!=0) && $data['timeend'] <= $data['timestart']) {
            $errors['timeend'] = get_string('timestartenderror', 'teamworkforum');
        }
        if (empty($data['message']['text'])) {
            $errors['message'] = get_string('erroremptymessage', 'teamworkforum');
        }
        if (empty($data['subject'])) {
            $errors['subject'] = get_string('erroremptysubject', 'teamworkforum');
        }
        return $errors;
    }
}
