...
/**
 * A Handler to process replies to teamworkforum posts.
 *
...
namespace mod_teamworkforum\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'mod_teamworkforum');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'mod_teamworkforum');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        $post = $DB->get_record('teamworkforum_posts', array('id' => $record->datavalue));
        if (!$post) {
        ...
        $cm = get_fast_modinfo($course->id)->instances['teamworkforum'][$teamworkforum->id];
        ...
        if (!$canpost) {
            $data = new \stdClass();
            $data->teamworkforum = $teamworkforum;
            throw new \core\message\inbound\processing_failed_exception('messageinboundnopostteamworkforum', 'mod_teamworkforum', $data);
        }
        ...
        if (!\core_availability\info_module::is_user_visible($cm)) {
            $data = new \stdClass();
            $data->teamworkforum = $teamworkforum;
            throw new \core\message\inbound\processing_failed_exception('messageinboundteamworkforumhidden', 'mod_teamworkforum', $data);
        }
        ...
        if (!empty($thresholdwarning) && !$thresholdwarning->canpost) {
            ...
            throw new \core\message\inbound\processing_failed_exception('messageinboundthresholdhit', 'mod_teamworkforum', $data);
        }

        $subject = clean_param($messagedata->envelope->subject, PARAM_TEXT);
        $restring = get_string('re', 'teamworkforum');
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (empty($teamworkforum->maxattachments) || $teamworkforum->maxbytes == 1 ||
                    !has_capability('mod/teamworkforum:createattachment', $modcontext)) {
                // Attachments are not allowed.
                mtrace("--> User does not have permission to attach files in this teamworkforum. Rejecting e-mail.");

                $data = new \stdClass();
                $data->teamworkforum = $teamworkforum;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundattachmentdisallowed', 'mod_teamworkforum', $data);
            }

            if ($teamworkforum->maxattachments < $attachmentcount) {
                // Too many attachments.
                mtrace("--> User attached {$attachmentcount} files when only {$teamworkforum->maxattachments} where allowed. "
                     . " Rejecting e-mail.");

                $data = new \stdClass();
                $data->teamworkforum = $teamworkforum;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilecountexceeded', 'mod_teamworkforum', $data);
            }
            ...
            if ($teamworkforum->maxbytes < $filesize) {
                // Too many attachments.
                mtrace("--> User attached {$filesize} bytes of files when only {$teamworkforum->maxbytes} where allowed. "
                     . "Rejecting e-mail.");
                $data = new \stdClass();
                $data->teamworkforum = $teamworkforum;
                $data->maxbytes = display_size($teamworkforum->maxbytes);
                $data->filesize = display_size($filesize);
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilesizeexceeded', 'mod_teamworkforum', $data);
            }
        }
        ...
        $addpost->id = teamworkforum_add_new_post($addpost, true);
        ...
        $event->add_record_snapshot('teamworkforum_posts', $addpost);
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/mod/teamworkforum/discuss.php', array('d' => $handlerresult->discussion));
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'mod_teamworkforum', $a);
        $message->html = get_string('postbymailsuccess_html', 'mod_teamworkforum', $a);
        return $message;
    }
}
