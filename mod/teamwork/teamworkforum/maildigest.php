...
/**
 * Set the mail digest option in a specific teamworkforum for a user.
 *
...
$teamworkforum = $DB->get_record('teamworkforum', array('id' => $id));
...
$url = new moodle_url('/mod/teamworkforum/maildigest.php', array(
    'id' => $id,
...
teamworkforum_set_user_maildigest($teamworkforum, $maildigest);
...
if ($maildigest === -1) {
    ...
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
        'mod_teamworkforum', $info);
    $updatemessage = get_string('emaildigestupdated_default', 'teamworkforum', $info);
} else {
    $info->maildigesttitle = $digestoptions[$info->maildigest];
    $info->maildigestdescription = get_string('emaildigest_' . $info->maildigest,
        'mod_teamworkforum', $info);
    $updatemessage = get_string('emaildigestupdated', 'teamworkforum', $info);
}
