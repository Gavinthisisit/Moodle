 ...
 */
class teamworkallocation_manual_renderer extends mod_teamwork_renderer  {
    ...
    protected function render_teamworkallocation_manual_allocations(teamworkallocation_manual_allocations $data) {
        ...
        $selfassessment     = $data->selfassessment;    // bool is the self-assessment allowed in this teamwork?
        ...
        $table->head        = array(get_string('participantreviewedby', 'teamwork'),
                                    get_string('participant', 'teamwork'),
                                    get_string('participantrevierof', 'teamwork'));
    ...
    /**
     * Returns information about the teamwork participant
     *
     * @return string HTML code
     */
    protected function helper_participant(stdclass $allocation, array $userinfo) {
        ...
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nosubmissionfound', 'teamwork'), 'info');
        } else {
            $link = $this->teamwork->submission_url($allocation->submissionid);
            ...
            if (is_null($allocation->submissiongrade)) {
                $o .= $this->output->container(get_string('nogradeyet', 'teamwork'), array('grade', 'missing'));
            } else {
                $o .= $this->output->container(get_string('alreadygraded', 'teamwork'), array('grade', 'missing'));
            }
        }
    ...
    protected function helper_reviewers_of_participant(stdclass $allocation, array $userinfo, array $reviewers, $selfassessment) {
        ...
        if (is_null($allocation->submissionid)) {
            ...
            if ($options) {
                ...
                $select = new single_select($handler, 'by', $options, '', array(''=>get_string('chooseuser', 'teamwork')), 'addreviewof' . $allocation->userid);
                $select->set_label(get_string('addreviewer', 'teamworkallocation_manual'));
