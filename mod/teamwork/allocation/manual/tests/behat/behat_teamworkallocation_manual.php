...
/**
 * Steps definitions related to teamworkallocation_manual.
 *
 * @package    teamworkallocation_manual
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_teamworkallocation_manual extends behat_base {
    /**
     * Manually adds a reviewer for teamwork participant.
     *
     * This step should start on manual allocation page.
     *
     * @When /^I add a reviewer "(?P<reviewer_name_string>(?:[^"]|\\")*)" for teamwork participant "(?P<participant_name_string>(?:[^"]|\\")*)"$/
     * @param string $reviewername
     * @param string $participantname
     */
    public function i_add_a_reviewer_for_teamwork_participant($reviewername, $participantname) {
        ...
        try {
            ...
            $this->find_button(get_string('showallparticipants', 'teamworkallocation_manual'))->press();
            $selectnode = $this->find('xpath', $xpathselect);
        }
        ...
        $allocatedtext = $this->getSession()->getSelectorsHandler()->xpathLiteral(
            get_string('allocationadded', 'teamworkallocation_manual'));
        $this->find('xpath', "//*[contains(.,$allocatedtext)]");
    }

    /**
     * Manually allocates multiple reviewers in teamwork.
     *
     * @When /^I allocate submissions in teamwork "(?P<teamwork_name_string>(?:[^"]|\\")*)" as:"$/
     ...
     */
    public function i_allocate_submissions_in_teamwork_as($teamworkname, TableNode $table) {

        $this->find_link($teamworkname)->click();
        $this->find_link(get_string('allocate', 'teamwork'))->click();
        $rows = $table->getRows();
        ...
        for ($i = 1; $i < count($rows); $i++) {
            $this->i_add_a_reviewer_for_teamwork_participant($rows[$i][$reviewer], $rows[$i][$participant]);
        }
    }
}
