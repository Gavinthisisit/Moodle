 ...
 *
 * @package    teamworkallocation_random
 ...
 * Allocates the submissions randomly
 */
class teamwork_random_allocator implements teamwork_allocator {
     ...
     *
     * @param teamwork_random_allocator_setting $setting
     * @param teamwork_allocation_result allocation result logger
     */
    public function execute(teamwork_random_allocator_setting $settings, teamwork_allocation_result $result) {

        $authors        = $this->teamwork->get_potential_authors();
        $authors        = $this->teamwork->get_grouped($authors);
        $reviewers      = $this->teamwork->get_potential_reviewers(!$settings->assesswosubmission);
        $reviewers      = $this->teamwork->get_grouped($reviewers);
        $assessments    = $this->teamwork->get_all_assessments();
        ...
        if ($settings->numofreviews) {
            ...
            $result->log(get_string('numofrandomlyallocatedsubmissions', 'teamworkallocation_random', count($randomallocations)));
        ...
        if ($settings->addselfassessment) {
            ...
            $result->log(get_string('numofselfallocatedsubmissions', 'teamworkallocation_random', count($selfallocations)));
            unset($selfallocations);
        }
        if (empty($newallocations)) {
            $result->log(get_string('noallocationtoadd', 'teamworkallocation_random'), 'info');
        } else {
            ...
            foreach ($newallocations as $newallocation) {
                ...
                if (isset($allreviewers[$reviewerid])) {
                    ...
                    if (!$allreviewersreloaded) {
                        $allreviewers = $this->teamwork->get_potential_reviewers(false);
                ...
                if (in_array($newallocation, $newnonexistingallocations)) {
                    $result->log(get_string('allocationaddeddetail', 'teamworkallocation_random', $a), 'ok', 1);
                } else {
                    $result->log(get_string('allocationreuseddetail', 'teamworkallocation_random', $a), 'ok', 1);
                }
            }
        }
        if ($settings->removecurrent) {
            ...
            // by reviewer
            $result->log(get_string('numofdeallocatedassessment', 'teamworkallocation_random', count($delassessments)), 'info');
            foreach ($delassessments as $delassessmentkey => $delassessmentid) {
                ...
                if (!is_null($assessments[$delassessmentid]->grade)) {
                    $result->log(get_string('allocationdeallocategraded', 'teamworkallocation_random', $a), 'error', 1);
                    unset($delassessments[$delassessmentkey]);
                } else {
                    $result->log(get_string('assessmentdeleteddetail', 'teamworkallocation_random', $a), 'info', 1);
                }
            }
            $this->teamwork->delete_assessment($delassessments);
        }
        $result->set_status(teamwork_allocation_result::STATUS_EXECUTED);
    }
     ...
     *
     * @param array $authors grouped of {@see teamwork::get_potential_authors()}
     * @param array $reviewers grouped by {@see teamwork::get_potential_reviewers()}
     * @param array $assessments as returned by {@see teamwork::get_all_assessments()}
    ...
    protected function add_new_allocations(array $newallocations, array $dataauthors, array $datareviewers) {
        ...
        $submissions    = $this->teamwork->get_submissions($authorids);
        $submissions    = $this->index_submissions_by_authors($submissions);
        foreach ($newallocations as $newallocation) {
            list($reviewerid, $authorid) = each($newallocation);
            if (!isset($submissions[$authorid])) {
                throw new moodle_exception('unabletoallocateauthorwithoutsubmission', 'teamwork');
            }
            $submission = $submissions[$authorid];
            $status = $this->teamwork->add_allocation($submission, $reviewerid, 1, true);   // todo configurable weight?
            if (teamwork::ALLOCATION_EXISTS == $status) {
        ...
        if (is_array($submissions)) {
            foreach ($submissions as $submissionid => $submission) {
                if (isset($byauthor[$submission->authorid])) {
                    throw new moodle_exception('moresubmissionsbyauthor', 'teamwork');
                }
     ...
     * @param teamwork_allocation_result $result allocation result logger
    ...
    protected function random_allocation($authors, $reviewers, $assessments, $result, array $options) {
        ...
        if (teamwork_random_allocator_setting::NUMPER_SUBMISSION == $numper) {
            // circles are authors, squares are reviewers
            $result->log(get_string('resultnumperauthor', 'teamworkallocation_random', $numofreviews), 'info');
            $allcircles = $authors;
            $allsquares = $reviewers;
            // get current workload
            list($circlelinks, $squarelinks) = $this->convert_assessments_to_links($assessments);
        } elseif (teamwork_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            $result->log(get_string('resultnumperreviewer', 'teamworkallocation_random', $numofreviews), 'info');
            $allcircles = $reviewers;
            $allsquares = $authors;
            // get current workload
            list($squarelinks, $circlelinks) = $this->convert_assessments_to_links($assessments);
        } else {
            throw new moodle_exception('unknownusertypepassed', 'teamwork');
        }
                    ...
                    while ($keeptrying && (count($circlelinks[$circleid]) < $requiredreviews)) {
                        ...
                        if (NOGROUPS == $gmode) {
                            if (in_array(0, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeers', 'teamworkallocation_random'), 'error', 1);
                                break;
                            }
                            $targetgroup = 0;
                            ...
                            if (in_array($circlegroupid, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeersingroup', 'teamworkallocation_random'), 'error', 1);
                                break;
                            }
                        ...
                        if ($targetgroup === false) {
                            $keeptrying = false;
                            $result->log(get_string('resultnotenoughpeers', 'teamworkallocation_random'), 'error', 1);
                            break;
                        }
        ...
        if (teamwork_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            foreach ($circlelinks as $circleid => $squares) {
                foreach ($squares as $squareid) {
     ...
     *
     * @param array $assessments array of assessments as returned by {@link teamwork::get_all_assessments()}
 ...
 */
class teamwork_random_allocator_setting {
     ...
     * @return teamwork_random_allocator_setting
     */
