 ...
 */
class teamwork_best_evaluation extends teamwork_evaluation {

    /** @var teamwork the parent teamwork instance */
    protected $teamwork;

    /** @var the recently used settings in this teamwork */
    protected $settings;

    /**
     * Constructor
     *
     * @param teamwork $teamwork The teamwork api instance
     * @return void
     */
    public function __construct(teamwork $teamwork) {
        global $DB;
        $this->teamwork = $teamwork;
        $this->settings = $DB->get_record('teamworkeval_best_settings', array('teamworkid' => $this->teamwork->id));
    }

    /**
     * Calculates the grades for assessment and updates 'gradinggrade' fields in 'teamwork_assessments' table
     *
    ...
    public function update_grading_grades(stdclass $settings, $restrict=null) {
        ...
        // Remember the recently used settings for this teamwork.
        if (empty($this->settings)) {
            $record = new stdclass();
            $record->teamworkid = $this->teamwork->id;
            $record->comparison = $settings->comparison;
            $DB->insert_record('teamworkeval_best_settings', $record);
        } elseif ($this->settings->comparison != $settings->comparison) {
            $DB->set_field('teamworkeval_best_settings', 'comparison', $settings->comparison,
                    array('teamworkid' => $this->teamwork->id));
        }

        // Get the grading strategy instance.
        $grader = $this->teamwork->grading_strategy_instance();
     ...
     *
     * @return teamwork_best_evaluation_settings_form
     */
    public function get_settings_form(moodle_url $actionurl=null) {
        ...
        return new teamwork_best_evaluation_settings_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public static function delete_instance($teamworkid) {
        ...
        $DB->delete_records('teamworkeval_best_settings', array('teamworkid' => $teamworkid));
    }
    ...
    protected function process_assessments(array $assessments, array $diminfo, stdclass $settings) {
        ...
        if (is_null($average)) {
            foreach ($assessments as $asid => $assessment) {
                if (!is_null($assessment->gradinggrade)) {
                    $DB->set_field('teamwork_assessments', 'gradinggrade', null, array('id' => $asid));
                }
            }
            return;
        }
        ...
        foreach ($grades as $assessmentid => $grade) {
            if (grade_floats_different($grade, $assessments[$assessmentid]->gradinggrade)) {
                ...
                $DB->update_record('teamwork_assessments', $record, true);  // bulk operations expected
            }
        }
 ...
 */
class teamwork_best_evaluation_settings_form extends teamwork_evaluation_settings_form {
    ...
    protected function definition_sub() {
        ...
        $plugindefaults = get_config('teamworkeval_best');
        $current = $this->_customdata['current'];

        $options = array();
        for ($i = 9; $i >= 1; $i = $i-2) {
            $options[$i] = get_string('comparisonlevel' . $i, 'teamworkeval_best');
        }
        $mform->addElement('select', 'comparison', get_string('comparison', 'teamworkeval_best'), $options);
        $mform->addHelpButton('comparison', 'comparison', 'teamworkeval_best');
