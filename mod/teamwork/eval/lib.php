 ...
 */
abstract class teamwork_evaluation {

    /**
     * Calculates grades for assessment and updates 'gradinggrade' fields in 'teamwork_assessments' table
     *
     ...
     *
     * @return teamwork_evaluation_settings_form
     */
    public function get_settings_form(moodle_url $actionurl=null) {

        $customdata = array('teamwork' => $this->teamwork);
        $attributes = array('class' => 'evalsettingsform');

        return new teamwork_evaluation_settings_form($actionurl, $customdata, 'post', '', $attributes);
    }

    /**
     * Delete all data related to a given teamwork module instance
     *
     * This is called from {@link teamwork_delete_instance()}.
     *
 ...
 */
class teamwork_evaluation_settings_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'general', get_string('evaluationsettings', 'mod_teamwork'));

        $this->definition_sub();

        $mform->addElement('submit', 'submit', get_string('aggregategrades', 'teamwork'));
    }
