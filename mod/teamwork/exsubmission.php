...
if ($id and $assess and $canassess) {
    ...
    require_sesskey();
    $asid = $DB->get_field('teamwork_assessments', 'id',
...
if ($edit and $canmanage) {
    ...
    $example        = file_prepare_standard_editor($example, 'content', $contentopts, $teamwork->context,
                                        'mod_teamwork', 'submission_content', $example->id);
    $example        = file_prepare_standard_filemanager($example, 'attachment', $attachmentopts, $teamwork->context,
                                        'mod_teamwork', 'submission_attachment', $example->id);

    $mform          = new teamwork_submission_form($PAGE->url, array('current' => $example, 'teamwork' => $teamwork,
...
if ($example->id) {
    ...
    echo $output->confirm(get_string('exampledeleteconfirm', 'teamwork'),
            new moodle_url($PAGE->url, array('delete' => 1, 'confirm' => 1)), $teamwork->view_url());
    }
    if ($canmanage and !$delete and !$DB->record_exists_select('teamwork_assessments',
            'grade IS NOT NULL AND weight=1 AND submissionid = ?', array($example->id))) {
        echo $output->confirm(get_string('assessmentreferenceneeded', 'teamwork'),
                new moodle_url($PAGE->url, array('assess' => 1)), $teamwork->view_url());
    }
...
if ($canmanage) {
    if (empty($edit) and empty($delete)) {
        $aurl = new moodle_url($teamwork->exsubmission_url($example->id), array('edit' => 'on'));
        echo $output->single_button($aurl, get_string('exampleedit', 'teamwork'), 'get');

        $aurl = new moodle_url($teamwork->exsubmission_url($example->id), array('delete' => 'on'));
        echo $output->single_button($aurl, get_string('exampledelete', 'teamwork'), 'get');
    }
}
// ...and optionally assess it
if ($canassess or ($canmanage and empty($edit) and empty($delete))) {
    $aurl = new moodle_url($teamwork->exsubmission_url($example->id), array('assess' => 'on', 'sesskey' => sesskey()));
    echo $output->single_button($aurl, get_string('exampleassess', 'teamwork'), 'get');
}
