 ...
 */
abstract class teamwork_rubric_assessment_form extends teamwork_assessment_form {
    ...
    public function validation($data, $files) {
        ...
        for ($i = 0; isset($data['dimensionid__idx_'.$i]); $i++) {
            if (empty($data['chosenlevelid__idx_'.$i])) {
                $errors['chosenlevelid__idx_'.$i] = get_string('mustchooseone', 'teamworkform_rubric'); // used in grid
                $errors['levelgrp__idx_'.$i] = get_string('mustchooseone', 'teamworkform_rubric');      // used in list
            }
        }
        return $errors;
    }
 ...
 */
class teamwork_rubric_list_assessment_form extends teamwork_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'teamworkform_rubric', $i+1);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $levelgrp[] = $mform->createElement('radio', 'chosenlevelid__idx_'.$i, '',
                        format_text($definition, $definitionformat, null, $teamwork->course->id), $levelid);
            }
 ...
 */
class teamwork_rubric_grid_assessment_form extends teamwork_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $numofcolumns = array_reduce($levelcounts, 'teamwork::lcm', 1);

        $mform->addElement('header', 'rubric-grid-wrapper', get_string('layoutgrid', 'teamworkform_rubric'));

        $mform->addElement('html', '<table class="rubric-grid">' . "\n");
        $mform->addElement('html', '<th class="header">' . get_string('criteria', 'teamworkform_rubric') . '</th>');
        $mform->addElement('html', '<th class="header" colspan="'.$numofcolumns.'">'.get_string('levels', 'teamworkform_rubric').'</th>');
