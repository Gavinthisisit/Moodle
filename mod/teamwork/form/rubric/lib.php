 ...
 */
class teamwork_rubric_strategy implements teamwork_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        $this->descriptionopts = array_merge(array('context' => $this->teamwork->context), $this->descriptionopts);
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $this->teamwork->context, 'teamworkform_rubric', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new teamwork_edit_rubric_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        if ($DB->record_exists('teamworkform_rubric_config', array('teamworkid' => $this->teamwork->id))) {
            $DB->set_field('teamworkform_rubric_config', 'layout', $layout, array('teamworkid' => $this->teamwork->id));
        } else {
            $record = new stdclass();
            $record->teamworkid = $this->teamwork->id;
            $record->layout     = $layout;
            $DB->insert_record('teamworkform_rubric_config', $record, false);
        }
        ...
        foreach ($data as $record) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('teamworkform_rubric', $record);
            } else {
                // exiting field
                $DB->update_record('teamworkform_rubric', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $this->teamwork->context, 'teamworkform_rubric', 'description', $record->id);
            $DB->update_record('teamworkform_rubric', $record);
            ...
            foreach ($record->levels as $level) {
                ...
                if (empty($level->id)) {
                    // new field
                    $level->id = $DB->insert_record('teamworkform_rubric_levels', $level);
                } else {
                    // exiting field
                    $DB->update_record('teamworkform_rubric_levels', $level);
                }
            }
        }
        $DB->delete_records_list('teamworkform_rubric_levels', 'id', $deletelevs);
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $this->teamwork->context->id, 'teamworkform_rubric', 'description',
        ...
        $formclassname = 'teamwork_rubric_' . $this->config->layout . '_assessment_form';
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id AS id, MIN(l.grade) AS min, MAX(l.grade) AS max, 1 AS weight
                  FROM {teamworkform_rubric} d
            INNER JOIN {teamworkform_rubric_levels} l ON (d.id = l.dimensionid)
                 WHERE d.teamworkid = :teamworkid
              GROUP BY d.id';
    ...
    public static function delete_instance($teamworkid) {
        ...
        $dimensions = $DB->get_records('teamworkform_rubric', array('teamworkid' => $teamworkid), '', 'id');
        $DB->delete_records_list('teamworkform_rubric_levels', 'dimensionid', array_keys($dimensions));
        $DB->delete_records('teamworkform_rubric', array('teamworkid' => $teamworkid));
        $DB->delete_records('teamworkform_rubric_config', array('teamworkid' => $teamworkid));
    }
    ...
    protected function load_fields() {
        ...
        $sql = "SELECT r.id AS rid, r.sort, r.description, r.descriptionformat,
                  ...
                  FROM {teamworkform_rubric} r
             LEFT JOIN {teamworkform_rubric_levels} l ON (l.dimensionid = r.id)
                 WHERE r.teamworkid = :teamworkid
    ...
    protected function load_config() {
        ...
        if (!$config = $DB->get_record('teamworkform_rubric_config', array('teamworkid' => $this->teamwork->id), 'layout')) {
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($this->teamwork->context->id, 'teamworkform_rubric', 'description', $id);
            }
        }
        $DB->delete_records_list('teamworkform_rubric', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link teamwork_edit_rubric_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        for ($i = 0; $i < $raw->norepeats; $i++) {
            ...
            $cook[$i]->teamworkid           = $this->teamwork->id;
            $cook[$i]->sort                 = $i + 1;
     ...
     * @return array [int dimensionid] => stdclass teamwork_grades record
     */
