 ...
 * @uses moodleform
 */
class teamwork_edit_comments_strategy_form extends teamwork_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'teamworkform_comments', $i+1));
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                                get_string('dimensiondescription', 'teamworkform_comments'), '', $descriptionopts);
        }

        $mform->registerNoSubmitButton('noadddims');
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'teamworkform_comments',
                teamwork_comments_strategy::ADDDIMS));
