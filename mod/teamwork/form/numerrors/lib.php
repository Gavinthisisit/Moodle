 ...
 *
 * @package    teamworkform_numerrors
 ...
 * @return bool
 */
function teamworkform_numerrors_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('teamworkform_numerrors', array('id' => $itemid ,'teamworkid' => $teamwork->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/teamworkform_numerrors/$filearea/$itemid/$relativepath";
 ...
 */
class teamwork_numerrors_strategy implements teamwork_strategy {
    ...
    /** @var teamwork the parent teamwork instance */
    protected $teamwork;
     ...
     * Constructor
     *
     * @param teamwork $teamwork The teamwork instance record
     * @return void
     */
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'teamworkform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new teamwork_edit_numerrors_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->numerrors; // data to be saved into {teamworkform_numerrors}
        $mappings   = $data->mappings;  // data to be saved into {teamworkform_numerrors_map}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('teamworkform_numerrors', $record);
            } else {
                // exiting field
                $DB->update_record('teamworkform_numerrors', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts, $PAGE->context,
                                                      'teamworkform_numerrors', 'description', $record->id);
            $DB->update_record('teamworkform_numerrors', $record);
        }
        ...
        foreach ($data->mappings as $nonegative => $grade) {
            ...
            if (isset($this->mappings[$nonegative])) {
                $DB->set_field('teamworkform_numerrors_map', 'grade', $grade,
                            array('teamworkid' => $this->teamwork->id, 'nonegative' => $nonegative));
            } else {
                $DB->insert_record('teamworkform_numerrors_map',
                            (object)array('teamworkid' => $this->teamwork->id, 'nonegative' => $nonegative, 'grade' => $grade));
            }
        }
        ...
        $sql = "DELETE FROM {teamworkform_numerrors_map}
                      WHERE (($insql nonegative > :maxnonegative) AND (teamworkid = :teamworkid))";
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'teamworkform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new teamwork_numerrors_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function save_assessment(stdclass $assessment, stdclass $data) {
        ...
        for ($i = 0; $i < $data->nodims; $i++) {
            ...
            if (empty($grade->id)) {
                // new grade
                $grade->id = $DB->insert_record('teamwork_grades', $grade);
            } else {
                // updated grade
                $DB->update_record('teamwork_grades', $grade);
            }
        }
    ...
    public function get_assessments_recordset($restrict=null) {
       global $DB;
        ...
        $sql = 'SELECT s.id AS submissionid,
                  ...
                  JOIN {teamwork_assessments} a ON (a.submissionid = s.id)
                  JOIN {teamwork_grades} g ON (g.assessmentid = a.id AND g.strategy = :strategy)
                 WHERE s.example=0 AND s.teamworkid=:teamworkid'; // to be cont.
        $params = array('teamworkid' => $this->teamwork->id, 'strategy' => $this->teamwork->strategy);
    ...
    public function get_dimensions_info() {
        ...
        $params = array('teamworkid' => $this->teamwork->id);
        $dimrecords = $DB->get_records('teamworkform_numerrors', array('teamworkid' => $this->teamwork->id), 'sort', 'id,weight');
     ...
     * @param int|null $teamworkid id of teamwork instance to check, checks all in case of null
     * @return bool
     */
    public static function scale_used($scaleid, $teamworkid=null) {
        return false;
    }
    ...
    public static function delete_instance($teamworkid) {
        ...
        $DB->delete_records('teamworkform_numerrors', array('teamworkid' => $teamworkid));
        $DB->delete_records('teamworkform_numerrors_map', array('teamworkid' => $teamworkid));
    }
    ...
    /**
     * Loads the fields of the assessment form currently used in this teamwork
     *
    ...
    protected function load_fields() {
        ...
        $sql = 'SELECT *
                  FROM {teamworkform_numerrors}
                 WHERE teamworkid = :teamworkid
                 ORDER BY sort';
        $params = array('teamworkid' => $this->teamwork->id);
    ...
    protected function load_mappings() {
        global $DB;
        return $DB->get_records('teamworkform_numerrors_map', array('teamworkid' => $this->teamwork->id), 'nonegative',
                                'nonegative,grade'); // we can use nonegative as key here as it must be unique within teamwork
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        $fs         = get_file_storage();
        foreach ($ids as $id) {
            $fs->delete_area_files($PAGE->context->id, 'teamworkform_numerrors', 'description', $id);
        }
        $DB->delete_records_list('teamworkform_numerrors', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link teamwork_edit_numerrors_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->numerrors    = array();          // to be stored in {teamworkform_numerrors}
        $cook->mappings     = array();          // to be stored in {teamworkform_numerrors_map}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->numerrors[$i]                        = new stdclass();
            $cook->numerrors[$i]->id                    = $raw->{'dimensionid__idx_'.$i};
            $cook->numerrors[$i]->teamworkid            = $this->teamwork->id;
     ...
     * @return array of filtered records from the table teamwork_grades
     */
    ...
    protected function update_peer_grade(stdclass $assessment) {
        ...
        if (!is_null($suggested)) {
            $this->teamwork->set_peer_grade($assessment->id, $suggested);
        }
        return $suggested;
    }
