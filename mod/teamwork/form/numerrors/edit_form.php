 ...
 * @uses moodleform
 */
class teamwork_edit_numerrors_strategy_form extends teamwork_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            ...
            $mform->addElement('hidden', 'dimensionid__idx_'.$i);   // the id in teamwork_forms
            $mform->setType('dimensionid__idx_'.$i, PARAM_INT);
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                    get_string('dimensiondescription', 'teamworkform_numerrors'), '', $descriptionopts);
        ...
        $mform->addElement('static', 'mappinginfo', get_string('maperror', 'teamworkform_numerrors'),
                                                            get_string('mapgrade', 'teamworkform_numerrors'));
        ...
        for ($i = 1; $i <= $totalweight; $i++) {
            ...
            $selects[] = $mform->createElement('static', 'mapdefault__idx_'.$i, '',
                                        get_string('percents', 'teamworkform_numerrors', floor(100 - $i * 100 / $totalweight)));
            $mform->addGroup($selects, 'grademapping'.$i, $i, array(' '), false);
            $mform->setDefault('map__idx_'.$i, '');
        }

        $mform->registerNoSubmitButton('noadddims');
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'teamworkform_numerrors',
                teamwork_numerrors_strategy::ADDDIMS));
