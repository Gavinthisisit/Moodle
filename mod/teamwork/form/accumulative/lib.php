 ...
 *
 * @package    teamworkform_accumulative
 ...
 * @return bool
 */
function teamworkform_accumulative_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('teamworkform_accumulative', array('id' => $itemid ,'teamworkid' => $teamwork->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/teamworkform_accumulative/$filearea/$itemid/$relativepath";
 ...
 */
class teamwork_accumulative_strategy implements teamwork_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'teamworkform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new teamwork_edit_accumulative_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->accumulative;  // records to be saved into {teamworkform_accumulative}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id         = $DB->insert_record('teamworkform_accumulative', $record);
            } else {
                // exiting field
                $DB->update_record('teamworkform_accumulative', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $PAGE->context, 'teamworkform_accumulative', 'description', $record->id);
            $DB->update_record('teamworkform_accumulative', $record);
        }
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'teamworkform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new teamwork_accumulative_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id, d.grade, d.weight, s.scale
                  FROM {teamworkform_accumulative} d
    ...
    public static function scale_used($scaleid, $teamworkid=null) {
        ...
        if (!is_null($teamworkid)) {
            $conditions['teamworkid'] = $teamworkid;
        }
        return $DB->record_exists('teamworkform_accumulative', $conditions);
    }
    ...
    public static function delete_instance($teamworkid) {
        ...
        $DB->delete_records('teamworkform_accumulative', array('teamworkid' => $teamworkid));
    }
    ...
    protected function load_fields() {
        ...
        $sql = 'SELECT *
                  FROM {teamworkform_accumulative}
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($PAGE->context->id, 'teamworkform_accumulative', 'description', $id);
            }
        }
        $DB->delete_records_list('teamworkform_accumulative', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link teamwork_edit_accumulative_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->accumulative = array();        // records to be stored in {teamworkform_accumulative}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->accumulative[$i]                     = new stdclass();
            $cook->accumulative[$i]->id                 = $raw->{'dimensionid__idx_'.$i};
            $cook->accumulative[$i]->teamworkid         = $this->teamwork->id;
