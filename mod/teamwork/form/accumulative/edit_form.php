 ...
 * @uses moodleform
 */
class teamwork_edit_accumulative_strategy_form extends teamwork_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                    get_string('dimensiondescription', 'teamworkform_accumulative'), '', $descriptionopts);
            ...
            $mform->addElement('modgrade', 'grade__idx_'.$i,
                    get_string('dimensionmaxgrade','teamworkform_accumulative'), null, true);
            $mform->setDefault('grade__idx_'.$i, 10);
            $mform->addElement('select', 'weight__idx_'.$i,
                    get_string('dimensionweight', 'teamworkform_accumulative'), teamwork::available_dimension_weights_list());
            $mform->setDefault('weight__idx_'.$i, 1);
        }

        $mform->registerNoSubmitButton('noadddims');
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'teamworkform_accumulative',
                teamwork_accumulative_strategy::ADDDIMS));
