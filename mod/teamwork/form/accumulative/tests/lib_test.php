...
require_once($CFG->dirroot . '/mod/teamwork/form/accumulative/lib.php');


class teamwork_accumulative_strategy_testcase extends advanced_testcase {
    /** teamwork instance emulation */
    protected $teamwork;

    /** @var testable_teamwork_accumulative_strategy instance of the strategy logic class being tested */
    protected $strategy;
    ...
    protected function setUp() {
        ...
        $teamwork = $this->getDataGenerator()->create_module('teamwork', array('strategy' => 'accumulative', 'course' => $course));
        $cm = get_fast_modinfo($course)->instances['teamwork'][$teamwork->id];
        $this->teamwork = new teamwork($teamwork, $cm, $course);
        $this->strategy = new testable_teamwork_accumulative_strategy($this->teamwork);
    }
 ...
 */
class testable_teamwork_accumulative_strategy extends teamwork_accumulative_strategy {
