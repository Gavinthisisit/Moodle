...
/**
 * mod_teamwork data generator class.
 *
 * @package    mod_teamwork
 * @category   test
 * @copyright  2013 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_teamwork_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        // Add default values for teamwork.
        $record = (array)$record + array(
            'strategy' => $teamworkconfig->strategy,
            'grade' => $teamworkconfig->grade,
            'gradinggrade' => $teamworkconfig->gradinggrade,
            'gradedecimals' => $teamworkconfig->gradedecimals,
            'nattachments' => 1,
            'maxbytes' => $teamworkconfig->maxbytes,
            'latesubmissions' => 0,
            'useselfassessment' => 0,
            'overallfeedbackmode' => 1,
            'overallfeedbackfiles' => 0,
            'overallfeedbackmaxbytes' => $teamworkconfig->maxbytes,
            'useexamples' => 0,
            'examplesmode' => $teamworkconfig->examplesmode,
            'submissionstart' => 0,
            'submissionend' => 0,
     ...
     *
     * @param int $teamworkid Teamwork instance id.
     * @param int $authorid Author user id.
     * @param stdClass|array $options Optional explicit properties.
     * @return int The new submission id.
     */
    public function create_submission($teamworkid, $authorid, $options = null) {
        global $DB;
        ...
        $id = $DB->insert_record('teamwork_submissions', $record);
    ...
    public function create_assessment($submissionid, $reviewerid, $options = null) {
        ...
        $id = $DB->insert_record('teamwork_assessments', $record);
