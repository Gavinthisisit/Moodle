...
class moss {
    ...
    protected function get_clear_utf8_content($file) {
        ...
        if (array_search($file_type, array('.pdf', '.rtf', '.odt', '.doc', 'docx','.zip'))) {
    ...
    protected function save_results($url, $configid) {
		...
		$files = $fs->get_area_files_by_user(303, 'mod_workshop', 'submission_attachment', false, $userid,'sortorder', false);
                foreach ($files as $file) {
